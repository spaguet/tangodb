...
- [ ] **F1:** `FinancePaymentsPage` — период, источник, способ оплаты; «Итого» пересчитывается; пустое состояние корректно
- [ ] CSV в `DataExportPage` без регрессии; прямой CSV в журнале не добавлять без отдельного требования
- [ ] **F2:** `FinancialDashboard` — MoM %, line 6 мес., pie/bar по split абон./перс.
- [ ] **F3:** новые клиенты за период, топ-5, заполняемость — без тяжёлых N+1 на клиенте
...
| F1 — Фильтры журнала платежей | ⏸ после SaaS или по запросу | Промт 2 |
| F2 — KPI и графики owner | ⏸ после F1 | Промт 3 |
| F3 — Расширенная аналитика dashboard | ⏸ после F2 | Промт 4 |
| 2 — UX-упрощения форм | ⏸ после gate/SaaS | Промт 5 |
| 3 — UX настроек модулей | ⏸ | Промт 6 |
| 4 — Guardians / pipeline | ⏸ по запросу | Промт 7 (только план) |
| F5–F6 — Expenses / Payroll | ⏸ по запросу | Промт 7 |
...
| F5/F6 — expenses / payroll (сначала план) | Промт 7 |
