...
export default function PersonalLessonsPanel({
  ...
  // Tabs: "book" (sale) or "view" (browse database)
  ...
  const handleTogglePaid = async (lesson: PersonalLesson) => {
    ...
    const res = await onUpdatePersonalPaid(lesson.row || 0, nextStatus, lesson.id);
    if (!res.success) {
  ...
  const handleDeleteLesson = async (lesson: PersonalLesson) => {
    ...
    const res = await onDeletePersonal(lesson.row || 0, lesson.id);
    if (!res.success) {
