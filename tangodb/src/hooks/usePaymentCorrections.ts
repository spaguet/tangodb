...
export function useUpdatePaymentMethod() {
  ...
  return useMutation({
    mutationFn: async (input: {
      ...
      if (!result?.success) {
        return { success: false as const, error: result?.error ?? "corrections.error.methodUpdateFailed" };
      }
      return {
        success: true as const,
        paymentId: result.payment_id,
...
export function useRemoveOrphanPaymentStorno() {
  const queryClient = useQueryClient();
  ...
  return useMutation({
    mutationFn: async (input: {
      ...
      const { data, error } = await supabase.rpc("remove_orphan_payment_storno", {
        p_storno_id: input.stornoId,
        p_reason_comment: input.reasonComment ?? null,
        p_idempotency_key: input.idempotencyKey ?? crypto.randomUUID(),
      });
      if (error) return { success: false as const, error: error.message };
      ...
      if (!result?.success) {
        return { success: false as const, error: result?.error ?? "corrections.error.removeStornoFailed" };
      }
      return {
        success: true as const,
        stornoId: result.storno_id,
