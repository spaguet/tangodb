  ...
  return (
    <div className="panel-card-stack max-w-2xl">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="text-base font-semibold text-ink-900">{t("settings.disciplines.title")}</h2>
          <p className="text-xs text-ink-500 mt-1">{t("settings.disciplines.subtitle")}</p>
        </div>
        <RequirePermission action="disciplines.write" mode="hide">
          <button
            type="button"
            onClick={() => setAddModalOpen(true)}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-gold-700 bg-gold-50 border border-gold-100 rounded-lg hover:bg-gold-100 transition-colors cursor-pointer shrink-0"
          >
