      ...
      {status && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
          ...
          <motion.div
            ...
            className="relative w-full max-w-md rounded-xl border border-ink-200 bg-white p-4 shadow-xl space-y-4"
          >
            <div className="flex items-center gap-2">
              <AlertTriangle className="w-5 h-5 text-amber-700" />
              <h3 className="text-base font-semibold text-ink-900">{t("venueCosts.paymentConfirm.title")}</h3>
            </div>
            <VenueRuleExpiryNotice status={status} compact />
            <p className="text-xs text-ink-600 leading-relaxed">{t("venueCosts.paymentConfirm.body")}</p>
