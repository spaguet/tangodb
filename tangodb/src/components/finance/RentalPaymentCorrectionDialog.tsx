...
import { useCorrectRentalPayment, useStornoRentalPayment } from "../../hooks/useRentalPaymentCorrections";
...
interface RentalPaymentCorrectionDialogProps {
  payment: RentalPaymentWithCorrectionMeta | null;
  open: boolean;
  onClose: () => void;
  toast: (msg: string, type?: "success" | "error" | "info") => void;
}

export default function RentalPaymentCorrectionDialog({
  payment,
  open,
  onClose,
  toast,
}: RentalPaymentCorrectionDialogProps) {
  const { t, formatDateTime } = useI18n();
  const stornoPayment = useStornoRentalPayment();
  const correctPayment = useCorrectRentalPayment();
  ...
  const canCorrect = useMemo(() => {
    ...
    return rentalPaymentCanCorrect(payment);
  }, [payment]);
  ...
  const handleSubmit = async () => {
    ...
    if (mode === "void") {
      ...
      toast(
        res.alreadyApplied
          ...
          : t("corrections.rental.voidSuccess", {
    ...
    toast(
      res.alreadyApplied
        ? t("corrections.payment.alreadyApplied")
        : t("corrections.rental.correctSuccess", {
            op: formatOperationNumber(res.operationNumber),
          }),
      res.alreadyApplied ? "info" : "success"
    );
  ...
  const counterparty = payment.renterDisplay ?? t("schedule.rental.blockTitle");
        ...
        >
          ...
          >
            <div className="flex items-start justify-between gap-3 p-4 border-b border-slate-100">
              <div>
                <p className="text-base font-semibold text-slate-900">{t("corrections.rental.title")}</p>
                <p className="text-xs text-slate-500 mt-1">{t("corrections.rental.subtitle")}</p>
              </div>
            ...
            <div className="p-4 space-y-4">
              <div className="rounded-xl bg-slate-50 p-3 space-y-2 text-sm">
                ...
                <div className="flex justify-between gap-3">
                  <span className="text-slate-500">{t("corrections.rental.renter")}</span>
                  <span className="text-slate-800 truncate">{counterparty}</span>
                </div>
                <div className="flex justify-between gap-3">
