...
const detailLabelCls = "text-[10px] font-semibold uppercase tracking-wider text-ink-500";
const detailValueCls = "text-sm text-ink-800";
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
            ...
            >
              ...
              <dl className="space-y-3 font-sans">
                        ...
                        {personalClientEntries.map((entry, index) =>
                          entry.id && canReadClients ? (
                            <button
                              ...
                              className="block text-left text-gold-700 hover:text-gold-800 hover:underline underline-offset-2 cursor-pointer transition-colors"
                            >
                              {entry.label}
                            </button>
                          ) : (
                ...
                {movedFromLabel && (
                  <div className="rounded-lg border border-gold-100 bg-gold-50/70 px-3 py-2 text-xs text-gold-800">
                    {movedFromLabel}
                  </div>
                )}
                ...
                {lesson.kind === "personal" && canShowPaidStatus(role) && (
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      ...
                      <dd className={detailValueCls}>
                        {lesson.paid === "yes" ? (
                          <span className="text-ink-600">{t("common.paidLabel")}</span>
                        ) : (
                          <span className="text-garnet-600">{t("common.unpaidLabel")}</span>
                        )}
                      </dd>
                    </div>
                  </div>
              ...
              {activePersonalClosure ? (
                <div className="rounded-lg border border-ink-100 bg-ink-50/10 p-3 space-y-2">
                  <p className="text-xs font-semibold text-ink-800">{t("venueCosts.closeLesson.closed")}</p>
                  {canReopenPersonal && (
                    <>
                ...
                {canDelete && (
                  <RequirePermission action={lesson.kind === "group" ? "schedule.write" : "personal_lessons.write"} context={permissionContext}>
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-1.5 py-2.5 bg-garnet-50 hover:bg-garnet-100 text-garnet-700 text-xs font-semibold uppercase tracking-wider rounded-lg transition-colors cursor-pointer"
                    >
