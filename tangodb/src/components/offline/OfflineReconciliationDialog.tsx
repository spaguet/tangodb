...
function OpRow({
  ...
  return (
    <div className="rounded-xl border border-ink-200 bg-white p-3 space-y-2 text-xs">
      <div className="flex items-start justify-between gap-2">
        <div>
          <p className="font-semibold text-ink-800">{op.clientDisplay}</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-[11px]">
        <div className="rounded-lg bg-ink-50 p-2">
          <p className="text-ink-500 uppercase text-[10px] font-semibold">
      ...
      {op.status === "conflict" ? (
        <div className="flex flex-wrap gap-2 pt-1">
          <button
            ...
            className="px-3 py-1.5 rounded-lg bg-gold-700 text-white font-semibold cursor-pointer hover:bg-gold-700"
          >
          ...
          <button
            ...
            className="px-3 py-1.5 rounded-lg border border-ink-200 text-ink-700 font-semibold cursor-pointer hover:bg-ink-50"
          >
          ...
          <button
            ...
            className="px-3 py-1.5 rounded-lg text-garnet-600 font-semibold cursor-pointer hover:bg-garnet-50"
          >
      ...
      {op.lastError ? <p className="text-garnet-600 text-[11px]">{op.lastError}</p> : null}
    </div>
  );
}
      ...
      {open ? (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4" role="dialog" aria-modal="true">
          ...
          <motion.div
            ...
            className="relative bg-white rounded-xl border border-ink-200 shadow-xl p-5 max-w-lg w-full max-h-[90vh] flex flex-col"
          >
            <div className="flex items-start justify-between gap-3 mb-3">
              <div>
                <h3 className="text-sm font-semibold text-ink-900">{t("offline.reconciliation.title")}</h3>
                <p className="text-xs text-ink-500 mt-1">{t("offline.reconciliation.description")}</p>
              </div>
              <button
                type="button"
                onClick={onClose}
            ...
            <div className="space-y-3 overflow-y-auto flex-1 min-h-0 pr-1">
              {ops.length === 0 && drafts.length === 0 ? (
                <p className="text-sm text-ink-500 text-center py-4">{t("offline.reconciliation.empty")}</p>
              ) : null}
              ...
              {drafts.length > 0 ? (
                <div className="rounded-xl border border-amber-200 bg-amber-50/10 p-3 space-y-2">
                  <p className="text-xs font-semibold text-amber-700 flex items-center gap-1.5">
                    <AlertTriangle className="w-3.5 h-3.5" />
                    {t("offline.reconciliation.paymentDraftsTitle")}
                  </p>
                  <p className="text-[11px] text-amber-700">{t("offline.reconciliation.paymentDraftsHint")}</p>
                  <ul className="space-y-1">
                    {drafts.map((d) => (
                      <li key={d.id} className="text-[11px] text-ink-700">
                        · {d.reminderLabel}
                      </li>
                    ))}
                  </ul>
              ...
              {allDone && ops.some((o) => o.status === "applied") ? (
                <p className="text-xs text-gold-700 flex items-center gap-1.5">
            ...
            <div className="flex justify-end gap-2 pt-4 mt-2 border-t border-ink-100">
              <button
                type="button"
                onClick={onClose}
                disabled={syncing}
                className="px-4 py-2 text-xs font-semibold text-ink-600 bg-ink-100 hover:bg-ink-200 rounded-lg cursor-pointer disabled:opacity-60"
              >
                {t("common.cancel")}
              </button>
              <button
                ...
                disabled={syncing}
                className="px-4 py-2 text-xs font-semibold text-white bg-gold-700 hover:bg-gold-800 rounded-lg cursor-pointer disabled:opacity-60 flex items-center gap-1.5"
              >
