      ...
      {open && invoiceId ? (
        <div className="fixed inset-0 z-[65] flex items-center justify-center p-4" role="dialog" aria-modal="true">
          ...
          <motion.div
            ...
            className="relative w-full max-w-2xl max-h-[85vh] overflow-hidden bg-white rounded-xl border border-ink-200 shadow-xl flex flex-col"
          >
            <div className="flex items-center justify-between gap-3 px-4 py-3 border-b border-ink-100">
              <div className="flex items-center gap-2 min-w-0">
                <FileText className="w-4 h-4 text-gold-700 shrink-0" />
            ...
            <div className="p-4 overflow-y-auto space-y-4 text-sm">
              {docQuery.isLoading ? (
                ...
                <p className="text-sm text-garnet-600">{t("rentalBilling.error.documentLoadFailed")}</p>
              ) : (
                <>
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <p>
                      <span className="text-ink-400">{t("rentalBilling.documentNumber")}:</span>{" "}
                      {doc.documentNumber ?? "—"}
                    </p>
                    <p>
                      <span className="text-ink-400">{t("rentalBilling.documentVersion")}:</span>{" "}
                      {doc.documentVersion}
                    </p>
                    <p>
                      <span className="text-ink-400">{t("rentalInvoices.period")}:</span>{" "}
                      {doc.periodStart} – {doc.periodEnd}
                    </p>
                    <p>
                      <span className="text-ink-400">{t("rentalInvoices.dueDate")}:</span> {doc.dueDate}
                    </p>
                  </div>

                  <div className="rounded-lg bg-ink-50 p-3 text-xs space-y-1">
                    <p className="font-semibold text-ink-800">{doc.organizationName}</p>
                  ...
                  <div className="text-xs">
                    <p className="font-semibold text-ink-800">{t("rentalBilling.renter")}</p>
