      ...
      <div className="bg-white rounded-xl border border-ink-200 shadow-xs overflow-hidden">
        <div className="px-4 py-3 border-b border-ink-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-sans text-sm font-semibold text-ink-800">{t("finance.revenue.title")}</h2>
          </div>
        ...
        <div className="p-4 space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-6 gap-3">
            <div className="bg-ink-50 rounded-lg px-3 py-2.5 border border-ink-100">
              <p className="text-[10px] text-ink-500 uppercase font-semibold tracking-wider">{t("finance.revenue.net")}</p>
              <p className="text-lg font-semibold text-ink-900 mt-0.5">{formatCurrency(stats.netTotal)}</p>
            </div>
            <div className="bg-ink-50 rounded-lg px-3 py-2.5 border border-ink-100">
              <p className="text-[10px] text-ink-500 uppercase font-semibold tracking-wider">{t("finance.revenue.gross")}</p>
              <p className="text-lg font-semibold text-ink-800 mt-0.5">{formatCurrency(stats.grossTotal)}</p>
            ...
            <div className="bg-ink-50 rounded-lg px-3 py-2.5 border border-ink-100">
              <p className="text-[10px] text-ink-500 uppercase font-semibold tracking-wider">{t("finance.revenue.refunds")}</p>
              <p className="text-lg font-semibold text-garnet-700 mt-0.5">−{formatCurrency(stats.refundsTotal)}</p>
              {stats.refundCount > 0 ? (
                <p className="text-[10px] text-ink-500 mt-0.5">{stats.refundCount}</p>
              ) : null}
            </div>
            <div className="bg-ink-50 rounded-lg px-3 py-2.5 border border-ink-100">
              <p className="text-[10px] text-ink-500 uppercase font-semibold tracking-wider">{t("finance.revenue.pendingRefunds")}</p>
              <p className="text-lg font-semibold text-amber-700 mt-0.5">{formatCurrency(pendingTotal)}</p>
              {pendingRefunds.length > 0 ? (
                <p className="text-[10px] text-ink-500 mt-0.5">{pendingRefunds.length}</p>
              ) : null}
            </div>
            <div className="bg-ink-50 rounded-lg px-3 py-2.5 border border-ink-100">
              <p className="text-[10px] text-ink-500 uppercase font-semibold tracking-wider">{t("dashboard.subscriptions")}</p>
              <p className="text-lg font-semibold text-gold-700 mt-0.5">{formatCurrency(stats.subscriptionTotal)}</p>
            </div>
            <div className="bg-ink-50 rounded-lg px-3 py-2.5 border border-ink-100">
              <p className="text-[10px] text-ink-500 uppercase font-semibold tracking-wider">{t("dashboard.personal")}</p>
              <p className="text-lg font-semibold text-gold-700 mt-0.5">{formatCurrency(stats.personalTotal)}</p>
            </div>
            <div className="bg-ink-50 rounded-lg px-3 py-2.5 border border-ink-100">
              <p className="text-[10px] text-ink-500 uppercase font-semibold tracking-wider">{t("dashboard.singleVisits")}</p>
              <p className="text-lg font-semibold text-gold-700 mt-0.5">{formatCurrency(stats.singleVisitTotal)}</p>
            </div>
            <div className="bg-ink-50 rounded-lg px-3 py-2.5 border border-ink-100">
              <p className="text-[10px] text-ink-500 uppercase font-semibold tracking-wider">{t("finance.revenue.other")}</p>
              <p className="text-lg font-semibold text-ink-800 mt-0.5">{formatCurrency(stats.otherTotal)}</p>
              <p className="text-[10px] text-ink-500 mt-0.5">{t("schedule.event.financeSection")}</p>
            </div>
            <div className="bg-ink-50 rounded-lg px-3 py-2.5 border border-ink-100">
              <p className="text-[10px] text-ink-500 uppercase font-semibold tracking-wider">{t("finance.revenue.rental")}</p>
          ...
          {Object.keys(stats.byMethod).length > 0 && (
            <div>
              <p className="text-[10px] text-ink-500 uppercase font-semibold tracking-wider mb-2">{t("finance.revenue.byMethod")}</p>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                {Object.entries(stats.byMethod).map(([method, amount]) => (
                  <div
                    key={method}
                    className="flex items-center justify-between px-3 py-2 bg-white rounded-lg border border-ink-100 text-sm font-sans"
                  >
          ...
          {stats.count === 0 && (
            <div className="py-12 text-center">
              <TrendingUp className="w-8 h-8 text-ink-300 mx-auto mb-3" />
              <p className="text-sm text-ink-500">{t("finance.revenue.empty")}</p>
            </div>
          )}
        </div>
      </div>
      ...
      {personalTariffRows.length > 0 ? (
        <div className="bg-white rounded-xl border border-ink-200 shadow-xs overflow-hidden">
          <div className="px-4 py-3 border-b border-ink-100">
            <h3 className="font-sans text-sm font-semibold text-ink-800">
              {t("finance.revenue.personalTariffs.title")}
            </h3>
          </div>
          <div className="hidden sm:grid sm:grid-cols-[minmax(0,1.4fr)_auto_auto] gap-3 px-4 py-2 bg-ink-50 border-b border-ink-100 text-[10px] uppercase tracking-wider font-semibold text-ink-500 font-sans">
          ...
          <ul className="divide-y divide-ink-100">
            {personalTariffRows.map((row) => (
              ...
              >
                ...
                <span className="text-ink-600 sm:text-right">
                  <span className="sm:hidden text-[10px] uppercase tracking-wider text-ink-500 font-semibold mr-2">
                    {t("finance.revenue.personalTariffs.countColumn")}
                  </span>
                  {row.countPaymentsNet}
                </span>
                <span className="font-semibold text-ink-800 sm:text-right min-w-[6rem]">
                  <span className="sm:hidden text-[10px] uppercase tracking-wider text-ink-500 font-semibold mr-2">
      ...
      {pendingRefunds.length > 0 ? (
        <div className="bg-white rounded-xl border border-ink-200 shadow-xs overflow-hidden">
          <div className="px-4 py-3 border-b border-ink-100 flex items-center gap-2">
            ...
            <h3 className="font-sans text-sm font-semibold text-ink-800">{t("finance.revenue.pendingRefundsTitle")}</h3>
          </div>
          <ul className="divide-y divide-ink-100">
            {pendingRefunds.map((refund) => (
              <li key={refund.id} className="px-4 py-3 flex items-center justify-between gap-3 text-sm">
                <div>
                  <p className="font-semibold text-ink-800">{formatCurrency(refund.amount)}</p>
                  ...
                  {refund.reason ? <p className="text-xs text-ink-500 italic mt-0.5">{refund.reason}</p> : null}
                </div>
                <button
                  type="button"
