-- Venue cost payment gate: evaluate rule against lesson date, not payment date.
-- Expired monthly rules still cover lessons through the end of the following calendar month.

BEGIN;

DROP FUNCTION IF EXISTS get_venue_cost_rule_status(date);
...
BEGIN
  ...
  IF venue_cost_payment_ack_required(v_org_id, p_lesson_date) THEN
    ...
    RETURN v_status
      || jsonb_build_object(
        ...
        'status', CASE
          WHEN COALESCE((v_status ->> 'status')::text, '') = 'expired_ack_required' THEN 'expired_ack_required'
          ELSE 'expired_ack_required'
        END,
        'success', true
      );
  END IF;
  ...
  v_status := venue_cost_status_for_org(
    v_org_id,
    COALESCE(p_lesson_date, COALESCE(p_at, current_date))
  );
  RETURN v_status
