...
-- Hotfix 20261001000001 may already exist on production with the same
-- (uuid, uuid, text, text) identity but argument names p_charge_id, p_lesson_id.
-- DROP so this definition binds p_lesson_id first (matches the SPA).
DROP FUNCTION IF EXISTS write_off_personal_lesson_debt(uuid, uuid, text, text);
DROP FUNCTION IF EXISTS get_personal_lesson_debt_trace(uuid, uuid);
