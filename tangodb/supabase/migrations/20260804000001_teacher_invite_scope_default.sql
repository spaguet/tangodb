-- Teacher invite scope: default access for teacher role + backfill empty scopes
...
SET scope = default_teacher_scope()
WHERE om.role = 'teacher'
  AND om.is_active = true
  AND NOT teacher_scope_has_access(om.scope);

UPDATE organization_invites oi
SET scope = default_teacher_scope()
WHERE oi.role = 'teacher'
  AND oi.accepted_at IS NULL
  AND oi.revoked_at IS NULL
  AND oi.expires_at > now()
  AND NOT teacher_scope_has_access(oi.scope);
