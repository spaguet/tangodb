-- Per-member override: create/edit schedule entries on past dates (meta.can_edit_past_schedule).

BEGIN;

CREATE OR REPLACE FUNCTION can_edit_past_schedule()
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT COALESCE((om.meta ->> 'can_edit_past_schedule')::boolean, false)
...
GRANT EXECUTE ON FUNCTION can_edit_past_schedule() TO authenticated, service_role;
