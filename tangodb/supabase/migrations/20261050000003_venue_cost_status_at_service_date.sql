-- Payment / status RPCs must evaluate the venue rule as of the lesson date.
-- Overriding acknowledgement_required from current_date made Aug 28 lessons
-- look expired when the operator paid them on Sep 2.
...
AS $$
DECLARE
  v_at date := COALESCE(p_lesson_date, current_date);
BEGIN
  RETURN COALESCE(
    (venue_cost_status_for_org(p_org_id, v_at) ->> 'acknowledgement_required')::boolean,
    false
  );
END;
$$;
...
BEGIN
  ...
  RETURN v_status || jsonb_build_object('success', true);
END;
$$;
