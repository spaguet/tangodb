-- Attendance: add "excused" status (respectful absence — no lesson deduction)
-- Prices: optional location binding for local tariffs
...
ALTER TABLE prices DROP CONSTRAINT IF EXISTS prices_organization_id_type_lessons_key;

ALTER TABLE prices
  ADD CONSTRAINT prices_location_fkey
  FOREIGN KEY (organization_id, location_id)
  REFERENCES locations (organization_id, id)
  ON DELETE SET NULL;

CREATE UNIQUE INDEX IF NOT EXISTS prices_org_type_lessons_global_idx
  ON prices (organization_id, type, lessons)
  WHERE location_id IS NULL;

CREATE UNIQUE INDEX IF NOT EXISTS prices_org_type_lessons_local_idx
