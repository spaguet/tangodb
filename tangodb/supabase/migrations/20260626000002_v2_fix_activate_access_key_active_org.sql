-- Fix activate_access_key: set active org inline (avoid nested RPC auth edge cases)
...
BEGIN
  ...
  IF v_key.key_type = 'demo' THEN
    ...
    VALUES (
      ...
      v_now, v_now + interval '30 days', v_now + interval '60 days', v_user_id
    )
    ...
    SET status = 'active', organization_id = v_org_id, activated_at = v_now,
        demo_expires_at = v_now + interval '30 days', data_purge_at = v_now + interval '60 days'
    WHERE id = v_key.id;
...
-- Keep 2-arg overload for direct client RPC calls
CREATE OR REPLACE FUNCTION activate_access_key(p_key_hash text, p_org_name text DEFAULT NULL)
RETURNS jsonb
LANGUAGE sql
SECURITY DEFINER
SET search_path = public, auth
AS $$
  SELECT activate_access_key(p_key_hash, p_org_name, NULL::uuid);
$$;
...
GRANT EXECUTE ON FUNCTION activate_access_key(text, text) TO service_role;
