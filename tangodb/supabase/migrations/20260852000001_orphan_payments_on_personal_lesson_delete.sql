-- Detach journal payments before deleting personal lessons (fix client_id trigger + source CHECK on FK SET NULL).

ALTER TABLE payments
  ADD COLUMN IF NOT EXISTS is_orphaned_journal_entry BOOLEAN NOT NULL DEFAULT false;
...
ALTER TABLE payments
  ...
  CHECK (
    num_nonnulls(subscription_id, personal_lesson_id, single_visit_id) >= 1
    OR is_orphaned_journal_entry = true
  );

CREATE OR REPLACE FUNCTION orphan_personal_lesson_payments(p_org_id uuid, p_lesson_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  SELECT COALESCE(pl.client_id1, pl.client_id2, pl.client_id3, pl.client_id4)
  INTO v_fallback_client
  ...
  SET
    client_id = CASE
      ...
      WHEN v_fallback_client IS NOT NULL THEN v_fallback_client
      ELSE p.client_id
    END,
    personal_lesson_id = NULL,
    is_orphaned_journal_entry = true
