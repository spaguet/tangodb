-- Fix org purge: CASCADE DELETE fires calendar sync triggers that INSERT into
-- calendar_sync_outbox while organizations row is already gone → FK violation
-- (calendar_sync_outbox_organization_id_fkey). Same pattern as audit_% disable.
...
BEGIN
  ...
  FOR r IN
    ...
    WHERE n.nspname = 'public'
      ...
      AND (
        t.tgname LIKE 'audit_%'
        OR t.tgname LIKE '%calendar_sync%'
      )
  LOOP
