-- Bulk retry dead calendar sync jobs (e.g. after token_revoked + OAuth reconnect)
...
BEGIN
  ...
  -- Drop dead rows superseded by an active queue row with the same dedupe key.
  WITH doomed AS (
    SELECT dead.id
  ...
  -- Multiple dead rows can share one dedupe key — keep the newest only.
...
REVOKE ALL ON FUNCTION retry_organization_calendar_sync_dead_jobs(text[]) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION retry_organization_calendar_sync_dead_jobs(text[]) TO authenticated;

-- Service-role helper for OAuth reconnect worker path
...
REVOKE ALL ON FUNCTION service_retry_calendar_sync_dead_jobs_for_orgs(uuid[], text[]) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION service_retry_calendar_sync_dead_jobs_for_orgs(uuid[], text[]) TO service_role;
