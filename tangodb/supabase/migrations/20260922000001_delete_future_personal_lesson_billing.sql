-- Future personal lessons: purge charges/payments on delete instead of blocking on net payment.
-- Past lessons keep the net-payment guard (storno required).

CREATE OR REPLACE FUNCTION purge_personal_lesson_financials(p_org_id uuid, p_lesson_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_deleted_batch int;
BEGIN
  -- Remove payment rows (storno/correction children first), then lesson/charge payments.
  FOR v_pass IN 1..12 LOOP
    ...
    WHERE p.organization_id = p_org_id
      AND (
        p.reverses_payment_id IN (
          SELECT p2.id
          FROM payments p2
          WHERE p2.organization_id = p_org_id
            AND (
              p2.personal_lesson_id = p_lesson_id
              OR p2.personal_lesson_charge_id IN (
                SELECT plc.id
    ...
    v_deleted := v_deleted + v_deleted_batch;

    DELETE FROM payments p
    WHERE p.organization_id = p_org_id
      AND (
        p.personal_lesson_id = p_lesson_id
        OR p.personal_lesson_charge_id IN (
          SELECT plc.id
          FROM personal_lesson_charges plc
          WHERE plc.organization_id = p_org_id
            AND plc.personal_lesson_id = p_lesson_id
...
REVOKE ALL ON FUNCTION purge_personal_lesson_financials(uuid, uuid) FROM PUBLIC, authenticated;
