...
Deno.serve(async (req) => {
  ...
  if (!(await checkRateLimit(`dev-console-key:ip:${clientIp}`, RATE_LIMIT, RATE_WINDOW_MS))) {
  ...
  if (insertError) {
    logEvent("dev_console_key_error", { code: insertError.code ?? "unknown" });
    return jsonResponse({ error: "Key generation failed" }, 500, req);
  }
  ...
  logEvent("dev_console_key_generated", { key_id: keyRow.id });
