...
Deno.serve(async (req) => {
  ...
  if (!(await checkRateLimit(`gcal-set-binding:ip:${clientIp}`, RATE_LIMIT, RATE_WINDOW_MS))) {
  ...
  if (activeBinding) {
    try {
      ...
      await stopWatchForBinding(admin, oauthConfig, "member", activeBinding.id as string);
    } catch (err) {
      logEvent("gcal_watch_stop_on_change_calendar_error", {
        binding_id: activeBinding.id,
  ...
  try {
    ...
    logEvent("gcal_watch_register_after_binding_error", {
      binding_id: newBinding.id as string,
      message,
    });
  }
