...
async function writeAuditLog(
  ...
  const { error } = await admin.from("audit_log").insert({
    ...
    table_name: "organization_google_calendar_bindings",
    operation: "UPDATE",
    row_id: params.rowId,
...
Deno.serve(async (req) => {
  ...
  if (!(await checkRateLimit(`gcal-set-org-binding:ip:${clientIp}`, RATE_LIMIT, RATE_WINDOW_MS))) {
  ...
  if (activeBinding) {
    ...
    await writeAuditLog(admin, {
      ...
      userId,
      action: "google_calendar_org_change_calendar",
      metadata: {
  ...
  const { data: newBinding, error: insertError } = await admin
    ...
    .insert({
      ...
      configured_by_member_id: memberId,
  ...
  if (insertError || !newBinding) {
    logEvent("gcal_set_org_binding_error", {
      user_id: userId,
  ...
  await writeAuditLog(admin, {
    ...
    userId,
    action: "google_calendar_org_set_binding",
    metadata: {
      google_account_id: googleAccountId,
      calendar_id: calendarId,
      calendar_name: calendarName,
  ...
  logEvent("gcal_org_binding_saved", {
    user_id: userId,
    organization_id: organizationId,
