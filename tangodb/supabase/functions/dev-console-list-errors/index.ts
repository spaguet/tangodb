...
Deno.serve(async (req) => {
  ...
  if (!checkRateLimit(`dev-console-errors:ip:${clientIp}`, RATE_LIMIT, RATE_WINDOW_MS)) {
  ...
  if (q) {
    query = query.or(
      `action.ilike.%${q}%,target_type.ilike.%${q}%,metadata->>message.ilike.%${q}%,metadata->>code.ilike.%${q}%`
    );
  }
