 ...
 * Runs JWT helper + SQL tests against local DATABASE_URL only.
 *
...
function runSql(file) {
  ...
  const result = spawnSync(
    "psql",
    [process.env.DATABASE_URL, "-v", "ON_ERROR_STOP=1", "-f", abs],
    {
      encoding: "utf8",
      cwd: root,
      shell: false,
...
function main() {
  if (!process.env.DATABASE_URL) {
    console.error(
      "DATABASE_URL is not set. Point it at local `supabase start`.",
    );
    process.exit(1);
  }
  ...
  const jwt = runSql("_hall_rent_test_jwt.sql");
  if (!jwt.ok) {
