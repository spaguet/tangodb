...
### DS-ATELIER-0 — Design System v2 «Atelier»: палитра и этап 1 (2026-08-17)

- **Дата:** 2026-08-17
- **Статус:** **реализовано** (2026-08-17, Промпт 9, подверсия `2.9.0`)
- **Решение:** Принята целевая палитра и порядок миграции из `tangodb_design_system_v2.md` §0–§6. Ключевые пункты:
  1. **Палитра Atelier — 6 семейств:** `ink` (нейтраль), `gold` (primary), `lavender` (вторичный акцент), `sage` (успех), `garnet` (ошибки/долги/destructive), `amber` (только пассивные warning-баннеры: `50` / `200` / `700`).
  2. **Темы этапа 1:** CRM `tangodb/` — **светлая** (`ink-50` фон, `white` карточки); `tangodb-dev-console/` — **тёмная** (`ink-950` / `ink-900`); `tangodb-landing/` — светлая. Полный dark mode CRM — **out of scope** v2 (отдельный этап).
  3. **Шрифты:** CRM и dev-console — **Inter**; display-шрифты Atelier (`Cormorant Garamond`, `Plus Jakarta Sans`) — только landing, отдельный этап, не блокирует палитру.
  4. **Маппинг legacy → Atelier:** `slate` → `ink` (по роли, §2.1, не 1:1 по суффиксу); `indigo` → `gold`; `sky` / `violet` / `blue` → `lavender`; `emerald` → `sage`; `rose` / `red` → `garnet`.
  5. **Прозрачность:** только три уровня — `/10`, `/40`, `/70`; произвольные `/30`, `/50`, `/60`, `/80`, `/90`, `/95` нормализуются по смыслу (§3).
  6. **Расписание (`scheduleColors.ts` + `LessonBlock`):** групповой — `gold`; персональный — `lavender`; мероприятие — `lavender` (тёмнее); аренда — `ink`; различение типа — цвет + **иконка** (`Users`, `User`, `CalendarPlus`, `Building2`); highlight ring → `gold-600`, долг → `garnet-500`.
  7. **Бейдж Subscription (dev-console):** `bg-lavender-900/70 text-lavender-300` (вместо `blue-900/50 text-blue-300`).
  8. **Gold на светлом:** filled CTA — `bg-gold-600 text-white` (hover `gold-700`); inline-ссылки и accent-текст — `text-gold-700` (hover `gold-800`), не `gold-600` (WCAG).
  9. **Подверсия этапа 1:** `2.9.0` (новый контур Atelier palette); bump `appVersion.ts` / `package.json` — только Промпт 9.
  10. **Документация:** `.cursor/docs/ai/design_system.md` заменяется содержимым §5 после завершения кода (Промпт 9), не на этапе 0. Источник спеки до закрытия — `tangodb_design_system_v2.md`.
- **Контекст:** Текущая палитра — дефолтный Tailwind (`indigo-600` CTA, `slate` нейтраль): узнаваемый SaaS-шаблон, 9 цветовых семейств в UI, дублирующие семантики (`rose`+`red`, три синих). Редизайн — дисциплинированная премиальная система без смены layout/spacing/типографики CRM.
- **Альтернативы:**
  1. Механический `slate-N` → `ink-N` по числу — отклонено: тёплый ramp не совпадает 1:1 по контрасту (§2.1).
  2. Полный перевод CRM в тёмный Atelier на этапе 1 — отклонено: текущий продукт — light CRM; токены `ink-900`/`ink-950` нужны для dev-console и backdrop.
  3. Cormorant в CRM — отклонено: читаемость таблиц, форм, `text-[10px]` меток.
  4. Сохранить `sky`/`violet`/`blue` как отдельные семейства — отклонено: декоративная мультихромия в расписании и бейджах.
  5. `amber` для emergency CTA в dev-console — отклонено: `OrgsPage` emergency → `garnet-600` filled.
  6. Обновить `design_system.md` до кода — отклонено: до Промпта 8 в коде остаются legacy-классы.
- **Исключения:** Telegram-кнопки `#229ED9` / `#1C82B4` — единственный разрешённый hex вне токенов (§1.5).
- **Откат:** Промпты 0–9 — инкрементальные коммиты (рекомендация §8: после 0, 2, 3, 6, 9); токены Atelier на шаге 1 добавляются **параллельно** со старыми, удаление legacy — только Промпт 8 после grep-чистки.
- **Почему так:** Фиксирует палитру и продуктовые границы до массовой замены классов, чтобы агент не «улучшал» цвета от себя. Светлый CRM + тёплые нейтрали соответствуют B2B-инструменту; один primary (`gold`) и один secondary (`lavender`) снижают визуальный шум. Semver `2.9.0` отражает новый UI-контур по VER-1.
- **Закрытие:** Промпты 0–9 выполнены; `design_system.md` обновлён; CRM `2.9.0`.
...
- **Решение:** Принята целевая архитектура из `tangodb_google_calendar_integration.md` §16 Этап 0, §19–§20. Ключевые пункты:
  ...
  8. **Аренда:** `rentals` не синхронизируются с преподавательским календарём в MVP; отдельный календарь зала/организации — будущий контур.
