...
- **Альтернативы:** (1) слать из Mini App напрямую; (2) только личка владельцу без группы; (3) оставить TOPUP-1 без алерта.
- **Почему так:** алерт через тот же outbox+worker, что уведомления арендатору; chat_id привязывается точечно (invite без chat_id → первое добавление бота, или совпадение `t.me/username`); не спамим произвольные группы.
