...
- **Дата:** 2026-09-14
- **Решение:** FIFO и worker T−24 `prepaid_charged` только для самообслуживания (`created_by IS NULL`). Слот персонала остаётся `active` с резервом 50% до `time_start` / `time_end`.
- **Контекст:** две CRM-брони по 250k с кошелька 500k давали «Доступно» 125k: ближайший слот списывал 50% и earmark остатка.
- **Альтернативы:** (1) earmark remainder и у `active`; (2) не показывать earmark в «Доступно».
- **Почему так:** admin-холд не истекает за 24 ч (в отличие от Mini App self-hold); staff cancel и так возвращает prepay.

### HALL-RENT-STAFF-DEBT — кассовая бронь персонала: кошелёк или долг Mini App (2026-09-14)

- **Дата:** 2026-09-14
- **Решение:** Staff CRM создаёт аренду только через «Аренда» / «Серия аренд». Eligible слот всегда промотится в `channel = miniapp`. FIFO: хватает на 50% → резерв и remainder после слота; не хватает → `lifecycle = debt`, `debt_amount = полная стоимость`. Staff может снять такой долг-слот до `time_start` (`hold_deleted`). Occupancy игнорирует `cancelled` / `auto_deleted` / `hold_deleted`. Кассовые uninvoiced/accrual не считают Mini App. Кнопка Mini App в CRM удалена (самообслуживание арендатора в Telegram без изменений).
- **Контекст:** Кнопка Mini App создавала `awaiting_payment` с `debt_amount = 0`; та же сумма висела кассовым «не выставлено в счёт». После удаления холда слот мог оставаться занятым. 2.11.18 не промотила короткий кошелёк, чтобы не сжечь 24ч-холд.
- **Альтернативы:** (1) оставить отдельную кнопку Mini App; (2) короткий кошелёк оставлять кассой.
- **Почему так:** один staff-путь; долг Mini App не сгорает за 24 ч; касса не дублирует обязательства кошелька.

### HALL-RENT-TOPUP-4 — одна кнопка пополнения: pending, затем чат (2026-09-13)

- **Дата:** 2026-09-13
- **Решение:** Слой 1 (FC1) без draft (FC6): одна CTA создаёт `pending` с `TDB-XXXX`, копирует текст с кодом и открывает чат студии. Пре-submit кнопка «отправить чек» и галочка «чек отправлен» не используются. Fallback — шит после submit и кнопка на карточке pending. Бот по-прежнему не пишет staff.
- **Контекст:** Две параллельные кнопки (чат без кода и заявка в CRM) давали чек без inbox или заявку без чека.
- **Альтернативы:** (1) FC6 `draft → чат → pending`; (2) загрузка чека в Mini App / Storage.
- **Почему так:** код заявки должен быть в тексте чата; Mini App не умеет вложить фото от имени пользователя; FC6 и upload — отдельный scope.

### HALL-RENT-STAFF-WALLET — кассовая бронь персонала и кошелёк Mini App (2026-09-13)

- **Дата:** 2026-09-13
- **Решение:** Если owner/director/admin создаёт кассовый слот (`create_rental` / серия) арендатору с `telegram_id` и на кошельке хватает на 50% всех новых дат, слот промотится в `channel = miniapp` (FIFO-резерв, remainder worker). Касса с `initial_payment`, счетом или без Telegram остаётся кассой. Accountant не промотится. Правка слота Mini App по-прежнему `update_rental`; кассовые cancel/payment Mini App запрещены.
- **Контекст:** Админ бронировал через «Аренда» (касса 2.5); кошелёк не резервировался, вторая половина после слота не списывалась.
- **Альтернативы:** (1) только UX — прятать кассу; (2) отдельный кассовый hold без смены channel.
- **Почему так:** те же 50/50 и worker, что самообслуживание; не создаём истекающий холд, если денег нет.
...
- **Дата:** 2026-09-10
- **Решение:** `telegram_chat_url` — только ссылка Mini App «отправь чек». `telegram_staff_alert_chat_url` — куда бот шлёт `staff_topup_submitted`. Bind webhook/Confirm идёт по staff URL. Смена ссылки чеков не сбрасывает привязку алертов. Пустой staff URL = fallback на receipt URL (совместимость). Миграция копирует текущий URL в staff-поле.
- **Контекст:** Студия шлёт чеки лично администратору, а уведомления CRM — в группу всех админов. Одно поле не давало развести чаты.
- **Альтернативы:** (1) оставить один чат; (2) слать алерты всегда владельцу CRM по membership.
- **Почему так:** владелец сам выбирает оба адреса; Mini App не меняется; уже привязанные студии не теряют bind.

### HALL-RENT-TOPUP-2 — бот дублирует алерт в чат для чеков (2026-09-09)

- **Дата:** 2026-09-09
- **Решение:** После «Отправить заявку в CRM» арендатор по-прежнему сам пишет в чат студии и прикладывает чек (HALL-RENT-TOPUP-1). Дополнительно worker шлёт короткий plain-text алерт в тот же чат (`organization_renter_channel.telegram_receipt_chat_id`, в т.ч. отрицательный group id). Enqueue staff best-effort: ошибка Telegram не откатывает `renter_submit_topup`. Если Bot API не может писать первым (нет `/start` / бота нет в группе) — заявка создана, outbox `gate_wait` → `skipped`; после `/start` staff-строки оживают. Invite-группа **не** биндится первой попавшейся: кандидат + подтверждение в CRM. `@username` хранится на карточке арендатора и обновляется с mint.
- **Контекст:** Владелец не видел заявку, пока арендатор не написал в чат; бот не умел писать в группу (CHECK `telegram_id > 0`, webhook игнорировал группы).
- **Альтернативы:** (1) слать из Mini App напрямую; (2) только личка владельцу без группы; (3) оставить TOPUP-1 без алерта; (4) авто-bind первой invite-группы.
- **Почему так:** алерт через тот же outbox+worker, что уведомления арендатору; публичный `t.me/username` биндится по совпадению; invite — только после Confirm, иначе чужая группа.

### HALL-RENT-SELF-6 — Mini App wallet payout leftover (2026-09-05)

- **Дата:** 2026-09-05
- **Решение:** Выдача денег с Mini App-кошелька — отдельная кассовая операция `wallet_payout`, не `topup_reversal` (сторно ошибочного пополнения) и не ledger `refund` (отмена слота → кошелёк). `refundable = wallet − Σ debt − (prepay+remainder) холдов awaiting_payment − remainder живых active/prepaid_charged` (не `spendable − full holds`: reserved 50 % уже вне spendable, иначе двойной зачёт). Один шаг, `operation_date` = сегодня TZ орг, закрытый период запрещает. Gate = `member_can_record_rental_payment()` (как staff topup). UI на вкладке Финансы профиля арендатора.
- **Контекст:** Арендатор пополнил баланс и просит вернуть неиспользованное. Бухгалтерски нельзя оголить обязательства по уже занятым слотам и нельзя создать новый долг выдачей.
- **Альтернативы:** (1) сторно конкретной заявки topup; (2) выдавать весь spendable; (3) двухшаговая заявка pending→paid.
- **Почему так:** Сторно — коррекция проводки, не выплата клиенту. Spendable ещё содержит деньги под remainder и холды. Один шаг совпадает с кассой студии; заявление и галочка выдачи — основание в CRM без генерации КО-2.
...
- **Дата:** 2026-09-02
- **Решение:** Оповещение владельца о заявке на пополнение — личное сообщение арендатора в чат студии (сумма + чек). Исходящий бот по-прежнему пишет только арендатору (`renter_telegram_outbox`). В CRM — inbox `/finance/renter-topup` с быстрым «Зачислить». QR в Mini App читается через Storage SELECT + `createSignedUrl` (не только SQL JWT GUC).
- **Контекст:** SQL `_renter_qr_signed_url` часто отдавал null/относительный URL; владелец не видел заявку в Telegram, потому что outbox не шлёт staff; чек живёт в чате студии.
- **Альтернативы:** (1) sendMessage бота в группу студии (нужен chat_id и бот в группе); (2) пуш/email staff; (3) публичный bucket QR.
- **Почему так:** чек уже должен идти в allowlist-чат; Telegram сам нотифицирует владельца; бот в группу — отдельный контур и риск спама.

### HALL-RENT-SELF-2 — Mini App временно в комплекте с CRM (2026-09-02)

- **Дата:** 2026-09-02
- **Решение:** Пока нет отдельной оплаты модуля Telegram Mini App. `renter_miniapp_addon_is_active` = организация `licensed` и (lifetime **или** активная ежемесячная подписка CRM). Демо 30 дней (`demo_active` / `demo_retention`) — модуль выкл. Таблица `organization_addons` и цена `config.renterMiniappAddon` сохранены для возврата платного add-on.
- **Контекст:** Модуль должен быть доступен всем, кто купил CRM; отдельная ежемесячная плата за Mini App мешала. Ежемесячная подписка на CRM ещё будет реализована в продукте — SQL-гейт уже её учитывает.
- **Альтернативы:** (1) авто-выдача `organization_addons` при activate CRM; (2) оставить ручную заявку add-on; (3) открыть Mini App и на демо.
- **Почему так:** один гейт на купленный CRM, без новой таблицы; демо не получает канал арендатора; вернуть платёж модуля можно, снова прочитав `organization_addons`.

### HALL-RENT-SELF-1 — цена Mini App add-on в payment config (2026-08-31)

- **Дата:** 2026-08-31
- **Решение:** Ежемесячная цена модуля — `platform_payment_methods.config.renterMiniappAddon.{amount,currency}`. Реквизиты оплаты те же, что у лицензии CRM; сумма модуля отдельная. Гейт без оплаты не меняется: `paused` или истёкший `period_end` → `renter_miniapp_addon_is_active` = false.
- **Контекст:** CRM не показывал стоимость и «/ месяц»; оператор не мог задать цену модуля отдельно от lifetime CRM.
- **Альтернативы:** (1) колонка цены в `organization_addons`; (2) дублировать amount во всех методах оплаты; (3) Stripe/автосписание.
- **Почему так:** публичный конфиг уже читает CRM; реквизиты общие; автосписания в спеке нет — ручная заявка + Inbox.

### HALL-RENT-SELF-0 — Самообслуживание арендаторов (Mini App) (2026-08-29)

- **Дата:** 2026-08-29
- **Решение:** Целевой контур зафиксирован в `.cursor/docs/ai/renter_telegram_miniapp.md`. Кабинет арендатора — отдельное Telegram Mini App на боте студии (не логин команды CRM). Кошелёк развивается из `rental_advances`; холд «ожидает оплаты» занимает слот; активация 50 % стоимости FIFO; отдельное приложение `tangodb-renter/`; JWT `actor=renter` вне `organization_members`; add-on модуля в Dev Console. Подверсия реализации — **2.9** (новый модуль, VER-1). Код ещё не начат.
- **Контекст:** Сценарий сдачи зала арендаторам с Telegram, разовой и 4-недельной бронью, антифродом холдов (50 % / 75 %). Текущий hall-rent 2.5 — только касса персонала.
- **Альтернативы:** (1) Урезанный CRM с ролью renter в `organization_members`; (2) вытеснение неоплаченного холда оплаченным арендатором; (3) бесконечная серия вместо пакета 4 недель; (4) QR как `data:image` в JSON настроек.
- **Почему так:** RLS команды нельзя давать арендатору; вытеснение отвергнуто в пользу холда + счётчика; пакет 4 недель и пересчёт по разовому тарифу защищают от скидки «надолго» с отменой; Storage с удалением старых QR не раздувает бакет.

### DS-ATELIER-0 — откат (2026-08-17)

- **Дата:** 2026-08-17
- **Статус:** **отклонено / откат** после релиза `2.9.0`
- **Решение:** Полный revert коммита `a14b252` (`9806320`). CRM остаётся на `2.8.8` и legacy-палитре `slate` + `indigo` (+ семантические `rose`/`amber`/…). Спека `tangodb_design_system_v2.md` удалена из репозитория.
- **Контекст:** Визуальная оценка Atelier на реальных экранах — дизайн не принят владельцем продукта.
- **Почему так:** Откат через `git revert` (без force-push), чтобы сохранить историю и возможность изучить diff позже.

### SUBSTITUTE-1 — Замена преподавателя на одно занятие (2026-09-06)

- **Дата:** 2026-09-06
- **Решение:** Разовая замена хранится в `lesson_occurrence_substitutes` (group: слот+дата; personal: урок), без смены `schedule_slots.teacher_member_id` / `personal_lessons.teacher_member_id`. Назначить могут owner/director/full admin и постоянный преподаватель этого урока. Заменяющий — активный `teacher`. Пересечение по времени блокируется. Доступ к журналу и деталям занятия — только на эту дату. Зарплата считается по ставкам заменяющего (`resolve_teacher_pay_rule` / `teacher_pay_rates`); постоянный педагог за это occurrence не получает. Google Calendar occurrence уходит в календарь проводящего. Fallback payroll по пакетным `payments` по-прежнему атрибутирует групповой абонемент слотовому педагогу (ограничение: пакет куплен заранее).
- **Контекст:** Два преподавателя бальных; у второго нет scope группы А, нельзя отметить журнал и нельзя поставить его проводящим на один день.
- **Альтернативы:** (1) временно менять `teacher_member_id` слота; (2) выдавать постоянный доступ к группе; (3) ручная корректировка зарплаты.
- **Почему так:** Слот остаётся «чьим» у постоянного педагога; замена — operational overlay. Scope группы не расширяется. Payroll идёт от conducting teacher, как в сценарии студии.

### VER-1 — Semver CRM v2 (`2.x.y`) и отображение в UI (2026-08-07)

- **Дата:** 2026-08-07
- **Решение:**
  1. **Формат:** `2.x.y` — первая цифра = major продуктовая линия (`crm_product_versions`, сейчас `v2`); вторая = подверсия (крупный модуль/блок); третья = микропатч (правки внутри подверсии).
  2. **Источник истины в коде:** `tangodb/src/lib/appVersion.ts` (`APP_VERSION`) синхронизирован с `tangodb/package.json` (`version`).
  3. **Текущая версия:** `2.10.0` — подверсия 2.10 = замена преподавателя на занятие (SUBSTITUTE-1).
  4. **UI:** версия в левом sidebar над копирайтом (`App.tsx`), тот же стиль (`text-[10px] text-slate-400 font-sans`).
  5. **Карта подверсий v2 (реконструкция по миграциям):**
     - `2.0` — платформа v2 (multi-tenant, лицензии, RBAC)
     - `2.1` — операционное ядро (клиенты, расписание, абонементы, персональные уроки)
     - `2.2` — финансы базовые (расходы, payroll, дебиторы)
     - `2.3` — SaaS self-service + Dev Console
     - `2.4` — расширенное расписание и абонементы (freeze, waitlist, возвраты)
     - `2.5` — аренда зала (rentals, счета, фискализация)
     - `2.6` — затраты зала + правила оплаты преподавателей + фин. аудит
     - `2.7` — Google Calendar (OAuth, outbox, worker, группы, webhooks, free/busy)
     - `2.8` — персональный тариф с длительностью (PL-TARIFF-1)
     - `2.9` — самообслуживание арендаторов (Telegram Mini App)
     - `2.10` — замена преподавателя на одно занятие (SUBSTITUTE-1)
     - `2.11` — ручная месячная подписка CRM + платформенный бот (контур `.cursor/docs/ai/crm_monthly_subscription_and_support_bot.md`; S1 = контракт цен v2)
- **Контекст:** Нужен понятный номер релиза для поддержки и лицензирования; в репозитории не было semver, только `crm_product_versions.code = v2` и npm `0.1.0`.
- **Альтернативы:** Версия только в БД; привязка к числу миграций; отдельный CHANGELOG без кода.
- **Почему так:** Semver в коде виден пользователю и агенту; вторая цифра отражает продуктовые вехи, третья — итерации внутри модуля без смены контракта.

### GCAL-0 — Архитектурные решения интеграции Google Calendar MVP (2026-08-06)

- **Дата:** 2026-08-06
- **Решение:** Принята целевая архитектура из `tangodb_google_calendar_integration.md` §16 Этап 0, §19–§20. Ключевые пункты:
  1. **Источник истины и направление:** CRM — единственный источник истины для уроков; MVP — односторонняя синхронизация CRM → Google (outbound-only). Изменения в Google Calendar не откатывают и не меняют урок в CRM; обратный импорт Google → CRM — только как inbox черновиков в будущем (§5 версия 3, §16 Этап 6), вне MVP.
  2. **Рабочий календарь:** по умолчанию отдельный календарь `TangoDB / <название организации>`, не основной личный Google Calendar. Личные события пользователя не читаются и не копируются в CRM.
  3. **Сущности в MVP:** синхронизируются `personal_lessons` (этап 2) и групповые occurrence из `schedule_slots` (этап 3). `calendar_event_sessions` / org-level календарь — этап 4 (Промпт 11). Free/busy и webhook/incremental sync — этап 5 (Промпты 12–13), только по отдельному продуктовому решению.
  4. **privacy_mode по умолчанию:** `initials` для новых `member_google_calendar_bindings` (имя + первая буква фамилии; без телефонов, сумм, долгов и внутренних заметок). Пользователь может переключить на `full_name` или `hidden` в настройках интеграции.
  5. **Политика отмены по умолчанию:** `delete` — удалять Google event при отмене/удалении урока в CRM. Режим `mark_cancelled` (префикс «Отменено:», `transparency: transparent`) — опциональная настройка позже; не использовать Google `status: cancelled` для видимой бизнес-пометки.
  6. **Горизонт материализации групповых occurrence:** 7 дней назад (восстановление недавних операций) / 90 дней вперёд; ежедневная задача продлевает горизонт на один день. Отдельное Google-событие на каждый occurrence, не RRULE-серия.
  7. **Timezone:** переиспользовать существующее `organization_settings.timezone` (default `Europe/Moscow`); новую настройку не создавать. Worker строит `dateTime` + IANA `timeZone` из локальных `DATE` + `HH:MM` CRM.
  8. **Аренда:** `rentals` не синхронизируются с преподавательским календарём в MVP; отдельный календарь зала/организации — будущий контур. **Снято GCAL-5 (2026-09-02):** отдельный org-binding `purpose=rentals`.
- **Контекст:** Этап 0 `tangodb_google_calendar_integration.md` — согласование решений перед кодом (аналог PL-0, SCH-0). Одноразовый ICS-импорт (`ics-to-personal-lessons.mjs`) — миграционный сценарий, не заменяет ongoing CRM → Google.
- **Альтернативы:**
  1. Двусторонняя синхронизация Google ↔ CRM в MVP — отклонено: ненадёжное определение урока по названию, риск дублей и неверных оплат (§2, §19).
  2. Запись в основной личный календарь — отклонено: смешение личных и рабочих событий, сложный disconnect (§3).
  3. `privacy_mode: full_name` по умолчанию — отклонено: лишняя утечка PII в внешний календарь; `hidden` — слишком бедный UX для преподавателя.
  4. `mark_cancelled` по умолчанию — отклонено: CRM хранит историю отмены; календарь преподавателя должен отражать фактическую занятость (§6 «Отмены»).
  5. Горизонт материализации на год вперёд — отклонено: лишняя нагрузка на API и quota; 90 дней достаточно с ежедневным продлением.
  6. Синхронизация аренды в преподавательский календарь — отклонено: аренда — отдельный бизнес-контур (§6 «Аренда»).
  7. Новая настройка timezone для интеграции — отклонено: `organization_settings.timezone` уже используется расписанием (`SCHEDULE_TZ.md`).
...
### GCAL-5 — Отдельный Google Calendar для аренды зала (2026-09-02)

- **Дата:** 2026-09-02
- **Решение:**
  1. Подтверждённые `rentals` синхронизируются outbound CRM → Google, как мероприятия, но **не** в календарь преподавателя и **не** обязательно в календарь мероприятий.
  2. `organization_google_calendar_bindings.purpose` = `events` | `rentals`; один активный binding на пару `(organization_id, purpose)`.
  3. Owner/director выбирает Google-аккаунт из своих подключённых (или подключает другой) и writable-календарь на этом аккаунте. Кнопка «Создать календарь аренды» делает `TangoDB / <орг> / rentals` без reuse префикса `TangoDB /` (иначе подхватится календарь уроков).
  4. Payload: заголовок `Аренда · локация · арендатор`, описание без сумм/телефона/`internal_comment`. Отмена/удаление — delete Google event.
  5. Mini App холды с `booking_status = confirmed` занимают зал и тоже уходят в этот календарь.
- **Контекст:** GCAL-0 п.8 откладывал календарь зала; студии нужен отдельный аккаунт или отдельный календарь на том же аккаунте.
- **Альтернативы:** писать аренду в `purpose=events`; писать в member bindings преподавателей; отдельная таблица bindings.
- **Почему так:** тот же org-binding/outbox/worker, без смешения с уроками и без дубля схемы.
...
### PL-TARIFF-0 — Персональный тариф: длительность, автосумма, раздельные платежи (2026-08-16)

- **Дата:** 2026-08-16
- **Решение:** Принята целевая модель из `tangodb_personal_tariff_duration_payments.md` (§2 инварианты, §7–§9). Ключевые пункты:
  ...
  3. **Backfill `duration_minutes`** существующих персональных тарифов: `NULL` = legacy fixed; авто-60 не ставить.
  ...
  5. **Billed = multiply-first** (SQL `ROUND` на `numeric` / JS в копейках). Запрещено: `price × (minutes/duration)` в IEEE `number`.
  6. **На уроке хранить `price_id`** с продажи — пересчёт billed при смене слота до оплаты.
  ...
  9. **Этап 1** включает `payer_client_id` (S12–S13, S18). Таблица charges — этап 2.
  ...
  14. **Restate без платежей + выбор тарифа в кассе** = явный пересчёт (S5c). Restate после частичной оплаты — только режим B (оплата текущей суммы).
  15. **Выручка этапа 3:** ключ строки = `price_id` ?? снимок `tariff_label/price/duration`, не только `price_id`.
  16. **Кап `amount ≤ remaining`** — в RPC для всех вызовов, не только UI.
- **Контекст:** Этап 0 `tangodb_personal_tariff_duration_payments.md` — согласование решений перед миграциями и UI (Промпт 0). Сейчас CRM `2.7.26`; последняя миграция на дату решения — `20260916000001_restate_personal_lesson_amount.sql`.
- **Альтернативы:**
  1. Ручной коэффициент «увеличить тариф в N» — отклонено: непредсказуемо, не аудируется.
  2. Hard-block при длительности ≠ тарифу — отклонено: студии режут 60/75/80 минут.
  3. Split 50/50 в этапе 1 — отклонено: нужна таблица charges; только при подтверждённой потребности (этап 2).
  4. Backfill duration = 60 для legacy — отклонено: пересчитает старые оплаты при повторном открытии.
  5. Оставить два overload RPC — отклонено: UI идёт в wrapper без partial; 4-param без venue ack.
  6. Bump версии в начале этапа 1 — отклонено: `2.8.0` только после закрытия критериев этапа 1 (Промпт 9).
- **Почему так:** Минимум ручных коэффициентов; billed выводится из факта урока; долг и платежи — по плательщику; журнал хранит снимок тарифа. Инварианты multiply-first и запрет эвристики `price === tariff.price` защищают от 399.99 и отката restate. Поэтапная поставка (1 → charges → выручка) снижает риск поломки кассы.
- **Следующие шаги:** Этап 1 закрыт (`2.8.0`, Промпт 9). Промпт 10 — только при подтверждённом split 50/50; Промпт 11 — выручка по тарифам.

### PL-TARIFF-1 — Этап 1 закрыт: подверсия 2.8.0 (2026-08-16)

- **Дата:** 2026-08-16
- **Решение:** Подверсия **2.8.0** открыта (VER-1: новый контур — персональный тариф с длительностью). Критерии готовности этапа 1 (§9 п.1–9) закрыты: прайс с обязательной длительностью, billed из слота, касса tariff/outstanding, дебиторка по плательщику, журнал со снимком, формулы в `personalTariffPricing.ts`, SQL-тесты RPC/view.
- **Контекст:** Промпт 9 `tangodb_personal_tariff_duration_payments_prompts.md` после Промптов 0–8.
- **Почему так:** Bump только после закрытия критериев (PL-TARIFF-0 п.11). Этапы 2–3 — микропатчи `2.8.y` или отдельный `x` при split.
