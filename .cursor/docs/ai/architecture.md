...
- `tangodb-renter/` — Telegram Mini App арендатора (React + Vite, порт 3002). Свой origin `https://tangodb-renter.vercel.app`. **Vercel project Root Directory = `tangodb-renter`** — иначе Git-деплой берёт корневой `vercel.json` и заливает CRM на домен кабинета. Бандл: только `VITE_SUPABASE_URL` + anon key; auth через Edge `renter-telegram-auth` (`initData` / Telegram ID; `user.username` → `renters.telegram_username`). Расписание: недельная сетка **как CRM** (`WeeklyOccupancyGrid`: колонки дней + блоки по длительности), логика занято/свободно/свои без PII (шаг брони 30 мин), постоянная аренда с разными часами по дням (`day_slots`) и выбор недели внутри окна occupancy (текущая пн–вс + две будущие, TZ организации); не импортирует `tangodb/src`. Пополнение: действующие QR из bucket `org-rental-qr` (Storage signed URL с клиента); одна кнопка `renter_submit_topup` создаёт pending с кодом `TDB-XXXX`, копирует текст и открывает чат студии (`telegram_chat_url`); бот студии **дублирует** короткий алерт в чат уведомлений CRM (`telegram_staff_alert_chat_url` → `telegram_receipt_chat_id`, `staff_topup_submitted` в `renter_telegram_outbox`, drain `renter-booking-worker`, group chat_id < 0). Чеки и алерты могут быть разными чатами. Если staff URL пуст — fallback на `telegram_chat_url`. Invite-группа алертов: кандидат + Confirm в CRM, не первая попавшаяся. Личка алертов: получатель шлёт `/start`; после Start отложенные staff-алерты оживают. Ошибка доставки алерта не откатывает заявку. Владелец зачисляет в `/finance/renter-topup`. Возврат неиспользованного кошелька — кнопка «Вернуть средства» на вкладке Финансы профиля арендатора (`preview_renter_wallet_payout` / `staff_renter_wallet_payout`); к выдаче только свободный остаток кошелька после долга Mini App, 100 % живых холдов и remainder активных броней.
...
- **Касса персонала → кошелёк:** `create_rental` / `create_rental_series` для арендатора с `telegram_id` промотят неоплаченный слот в `channel = miniapp`. Хватает `available` на 50% — резерв (`active`) до начала слота, remainder после `time_end` (T−24 `prepaid_charged` только у самообслуживания, `created_by IS NULL`); не хватает — `lifecycle = debt` на полную стоимость (не кассовый uninvoiced). Accountant без `member_can_manage_rentals()` остаётся кассой 2.5. Оплата при создании, счета и арендаторы без Telegram — касса. Occupancy: `_rental_occupies_hall` (confirmed и не `cancelled`/`auto_deleted`/`hold_deleted`). Helper `_renter_attach_wallet_to_staff_cashier_rentals`.
...
- **Org overrides (§9):** boolean-флаги в `organization_settings` (`teachers_can_*`, `admin_can_*`); читаются в `permissions.ts` через `permissionOptionsFromSettings()` и в SQL через `can_export_data()`. `admin_can_manage_renter_balance` — узкий wallet-scope полного admin (`member_can_manage_renter_balance()`, UI `renters.balance.write`), не путать с `renters.finance.read`.
