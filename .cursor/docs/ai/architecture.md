...
- **Claim:** RPC `claim_calendar_sync_jobs` — атомарный lease, `FOR UPDATE SKIP LOCKED`, дедуп по `(organization_id, dedupe_key)`; если у просроченного `processing` lease уже есть более новая `pending/retry` строка, stale lease удаляется до возврата остальных задач в `retry`.
- **Strict-scope recovery:** при Google 404 на первичном insert и наличии `calendar.app.created` worker создаёт выделенный `TangoDB / <организация>`, через optimistic binding claim обновляет `calendar_id` и повторяет insert.
