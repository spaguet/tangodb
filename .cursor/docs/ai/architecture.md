...
- `src/components/schedule/` — недельная сетка расписания (Промпты 2–8):
  ...
  - **Замена преподавателя (2.10 / SUBSTITUTE-1):** `lesson_occurrence_substitutes` + RPC `assign_lesson_substitute` / `clear_lesson_substitute`. Постоянный педагог слота/урока не меняется. Заменяющий видит занятие и журнал только на эту дату, закрытие и payroll идут на conducting teacher (ставки заменяющего). Хуки `useLessonSubstitutes`; UI — `SubstituteLessonDialog` из карточки урока.
