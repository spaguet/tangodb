2026-09-12 — feat(billing): микропатч **2.11.6** (S3b) — витрина покупки CRM: SKU → способ → server quote → реквизиты/сумма из quote → заявка (`quote_id` + `client_request_id`); панель для demo, monthly renewal и suspended recovery, не lifetime; `?plan=` только предвыбор; CTA owner/director на past_due/suspended. SHA-256 контракта оплаты без `node:crypto` (Vite production). T−7 — S4.
