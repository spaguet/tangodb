...
2026-09-17 — fix(settings): микропатч **2.11.39** — UX15 аудит CRM UX: настройки «Общие / Организация / Ещё» на мобилке; «Сохранить» над таббаром; символ валюты org в селекте; подсказка admin без `settings.manage`; команда — люди первыми, приглашение свёрнуто, восстановление по «?», журнал без UUID; арендаторы — список и кнопка «Добавить», a11y полей; смена org сохраняет URL + toast при недоступной «Команда».

2026-09-17 — fix(finance): микропатч **2.11.38** — UX14 аудит CRM UX: навигация «Платежи / Кто должен / Ещё» (аренда и Mini App только при активном addon); журнал платежей без блока сдачи зала без модуля; KPI дебиторки на финансовом обзоре = `count · sum` как на операционном.

2026-09-17 — fix(prices): микропатч **2.11.37** — UX13 аудит CRM UX: один ряд вкладок «Услуги / Архив / Аренда зала»; карточки тарифов в режиме просмотра, цена и привязки — по «Изменить» в модалке; символ валюты org в поле стоимости.

2026-09-17 — fix(personal-lessons): микропатч **2.11.36** — UX12 аудит CRM UX: сортировка «от начала месяца» по умолчанию; преподаватель через `memberListLabel` / «Не назначен»; вкладка «Записать урок»; фильтры зала/направления в «Ещё фильтры»; отступ списка над таббаром; дата удаления в RU.

2026-09-17 — fix(attendance): микропатч **2.11.35** — UX11 аудит CRM UX: список уроков дня выше календаря; автовыбор единственного зала, черновики залов с пометкой/скрытие пустых; заголовок модалки с направлением; ученики и «Пришёл» в первом кадре; разовое и закрытие аренды зала ниже и только при настроенных venue costs; честная легенда прогула.

2026-09-17 — fix(schedule): микропатч **2.11.34** — UX10 аудит CRM UX: сетка в первом кадре (автораскрытие основного зала), мобильный режим «день» + PNG/Мероприятие/Аренда/Отпуск под сеткой; подпись урока в ячейке (не только время); пустые тестовые залы скрыты, черновик в названии; аренда в тулбаре только при модуле `locations`.

2026-09-17 — fix(subscriptions): микропатч **2.11.33** — UX09 аудит CRM UX: действующие абонементы раскрыты по умолчанию, фильтры в `<details>`; касса — авто «локальный прайс», дефолт направления с тарифами, дисциплина перед тарифом, честный empty-state + ссылка на `/prices`; CTA «Принять оплату», sticky над таббаром.

2026-09-17 — fix(clients): микропатч **2.11.32** — UX08 аудит CRM UX: список и поиск выше формы на мобилке; скрыты импорт-«(ID…) Удалён»; единый `formatClientName` (без «— Имя»); карточка с телефоном/абонементом/долгом, CTA «Продать»/«Написать», вкладка заметок; плейсхолдер телефона по валюте org; метка для служебных ФИО; касса `?client=`.

2026-09-17 — fix(dashboard): микропатч **2.11.31** — UX07 аудит CRM UX: единая дебиторка (`useDashboardDebtorMetrics` / группировка как в «Финансы → Дебиторы»); Mini App только при активном addon, свёрнут по умолчанию, скрыт на вкладке «Финансовый»; без вспышки «0 дебиторов»; убран «Пересчитать аренду» с финансового обзора; KPI с подсказками; «соло/парный», RU pending.

2026-09-17 — fix(shell): микропатч **2.11.30** — UX06 аудит CRM UX: `#boot-error` только при реальной ошибке (без Telegram в обычном web); skeleton при загрузке workspace; сброс `document.title` при выходе; порядок drawer (журнал → расписание → … → арендаторы в конце); «Записать персональный»; отступ контента над мобильным таббаром.

2026-09-17 — feat(onboarding): микропатч **2.11.29** — UX05 аудит CRM UX: визард (дефолт «частный преподаватель», часовой пояс из браузера, черновик в localStorage, подсказки шагов); после визарда «Зал 1» + направление; чек-лист 8 шагов на Обзоре; арендаторы скрыты без модуля «локации»; код восстановления по-русски с галочкой «сохранил».

2026-09-17 — fix(auth): UX04 аудит CRM UX — multi-org: выбор студии после Remember-me restore; `organizationPublicName` (branding_name) в select-org и OrgSwitcher; роли/статусы по-русски; CTA «не я» / ключ / новая студия; «Все организации» только при 3+ org; `document.title` на `/login` и `/select-organization`; «Запомнить меня» по умолчанию вкл.

2026-09-17 — fix(auth): UX03 аудит CRM UX — экран «Проверьте почту» (resend, спам, тема письма, help-аккордеон); повторная регистрация на занятый email — явная ошибка; `/register` при сессии — карточка «продолжить / не я / новая студия» (`RegisterRoute`); `resendSignupConfirmation` в AuthProvider.

2026-09-17 — fix(auth): UX02 аудит CRM UX — `/register`: компактная карточка, «Ваше имя» / «Эл. почта», подсказка правил пароля, Unicode-буквы в `validateAuthPassword`, Turnstile compact + RU «Проверка пройдена» вместо Success!, блок «Не получается зарегистрироваться?», `document.title`, `htmlFor` в `AuthField`.

2026-09-17 — fix(landing): UX01 аудит CRM UX — лендинг `#demo`: scroll-margin под шапку, непрозрачное мобильное меню, короткий таббар, RU-данные и текущий месяц, подпись «пример, не кликается» у hero-превью; версия демо = `APP_VERSION`.

2026-09-16 — fix(hall-rent): микропатч **2.11.28** — оплата счёта арендатора: снята дублирующая перегрузка `record_rental_invoice_payment` (ambiguous RPC после fiscal); технические сообщения Postgres о выборе функции больше не показываются в тосте. Миграция `20261129000001`.

2026-09-16 — fix(i18n): без смены версии — тост «Счёт создан на …» и «Максимум» в распределении аванса (`{{amount}}` вместо `{amount}`).

2026-09-16 — copy(renters): без смены версии — «Финансы» арендатора: кнопки и действия внутри контуров (янтарь/индиго), пояснения к счёту и распределению; убрана дублирующая «Принять аванс» (остаётся «Зачислить на кошелёк»).

2026-09-16 — copy(renters): без смены версии — вкладка «Финансы» арендатора: блоки «Касса» / «Mini App» с подсказками и легенда кнопок.

2026-09-16 — fix(hall-rent): без смены версии CRM — `record_rental_advance` синхронизирует остаток аванса в кошелёк (как R1b backfill); one-shot backfill для зависших авансов; подсказка в модалке «Принять аванс»; «Начислено к оплате (касса)». Миграция `20261128000008`.

2026-09-16 — copy(renters): без смены версии — подписи «Начислено / Оплачено / Переплата (касса)» в карточке арендатора (раньше «Сумма аренд» вводила в заблуждение: только канал cashier, не Mini App).
...
2026-09-14 — fix(hall-rent): микропатч **2.11.22** — staff-бронь в окне T−24 не списывает 50% сразу: оба слота только резерв, остаток кошелька = баланс − Σ50%. Самообслуживание арендатора по-прежнему prepaid_charged. Backfill возвращает ошибочно списанные staff-слоты в `active`. Миграция `20261127000001`.

2026-09-14 — fix(hall-rent): микропатч **2.11.21** — бронь персонала «Аренда» / «серия»: кошелёк Mini App резервирует 50% или сразу долг Mini App на полную стоимость (не кассовое «не выставлено в счёт»); удалённый холд не занимает зал; кнопка Mini App в CRM убрана. Миграция `20261126000001`.

2026-09-14 — feat(finance): микропатч **2.11.20** — в дебиторке фильтр периода: весь период / год (выбор года) / месяц (выбор месяца); сумма «К оплате» и список считаются по выбранному диапазону.
...
2026-09-13 — fix(hall-rent): микропатч **2.11.18** — кассовая бронь персонала для арендатора Mini App с достаточным кошельком сразу резервирует 50% и списывает остаток после слота (`_renter_attach_wallet_to_staff_cashier_rentals`); backfill уже созданных неоплаченных слотов. Кассовые cancel/payment Mini App снова запрещены. Миграция `20261125000001`.

2026-09-13 — feat(hall-rent): микропатч **2.11.17** — в попапе Mini App аренды у владельца/директора/админа кнопка «Изменить слот» (`EditRentalSlotModal`); `update_rental` разрешён для channel miniapp при `member_can_manage_rentals()`. Миграция `20261124000001`.

2026-09-13 — fix(personal-lessons): микропатч **2.11.16** — при смене тарифа в форме продажи не удаляются уже заполненные строки второго/третьего клиента (`applyLessonTariff`).

2026-09-13 — feat(hall-rent): микропатч **2.11.15** / Mini App **0.1.20** — необязательное «назначение аренды» при брони (Mini App + CRM miniapp-диалог); в расписании сначала назначение, затем имя арендатора; правка назначения в «Изменить слот». Миграция `20261123000001` (`purpose` на слоте через `_renter_insert_occurrence`).

2026-09-13 — fix(schedule): микропатч **2.11.14** — PNG расписания крупнее: ячейки 16px (было 12), заголовок/дни/время тоже увеличены, подпись по-прежнему переносится.
2026-09-13 — feat(crm): микропатч **2.11.13** — владелец/директор/админ могут переименовать арендатора на карточке (отображаемое имя) и править ФИО контакта; новое имя сразу в расписании (`display_name` живой join). PNG расписания: шрифт ячеек 12px и перенос подписи на следующие строки вместо обрезки «…».
...
2026-09-12 — fix(hall-rent): Mini App **0.1.17** — в форме «Забронировать» (разовая аренда) дата показывается как «03 июля 2026» (`formatLongDate`), а не ISO `YYYY-MM-DD`.
...
2026-09-12 — fix(personal): микропатч **2.11.11** — создание персонального урока на двух клиентов больше не теряет второго клиента при выборе оплаты/тарифа: `applyLessonTariff` добавляет недостающие поля, но не удаляет уже заполненные строки клиентов. Урок сохраняется и отображается с `client_id2`.
...
2026-09-12 — feat(billing): микропатч **2.11.10** (S6) — `platform_support_tickets` + RPC `submit_platform_support_ticket` / `dev_console_update_support_ticket`; Edge `submit-support-ticket` (гости: Turnstile fail-closed, HMAC rate limit, нейтральный ответ) и JWT для `license_help`/`other`; outbox `source_type=support_ticket` с ссылкой на ticket в Inbox; CRM формы login/forgot/шапка/license; Dev Console Inbox вкладка Support; i18n `support.ticket.*`; SQL-тест `crm_monthly_subscription_s6_test.sql`.

2026-09-12 — feat(billing): микропатч **2.11.9** (S5b) — Dev Console `/platform-bot`: Detect (`getMe` + one-shot `getUpdates`, отказ если webhook), paste/Save `telegram_chat_id`, Send test, Requeue blocked; `org_created` в той же TX, что INSERT self-service demo (dedupe `org_created:<org_id>`); ежедневный digest из SQL-source S4 в outbox из `expire-crm-subscriptions`. Без webhook. Username бота в CRM не показывается.
2026-09-12 — feat(billing): микропатч **2.11.8** (S5a) — `platform_notification_settings` + `platform_notification_outbox`; Edge `platform-notification-worker` (claim/lease, backoff, 403→blocked); `submit-purchase-request` только enqueue (email+Telegram в одной TX с заявкой); без token/chat_id Telegram=`blocked`/`config_missing`, заявка жива. Общий `sendTelegramMessagePlain`. Detect UI / org_created — S5b.

2026-09-12 — feat(billing): микропатч **2.11.7** (S4) — cron `expire-crm-subscriptions` + RPC `expire_crm_organization_subscriptions` (active→past_due, grace 7д → canceled+suspended, SKIP LOCKED); SQL-source digest без отправки; `isReadOnly` по `period_end` без ожидания cron; T−7 баннер/CTA owner/director; копирайт `ReadOnlyBanner` для истекшей подписки на CRM. Mini App гаснет в `period_end`. Outbox/Telegram — S5a.

2026-09-12 — feat(billing): микропатч **2.11.5** (S3a) — `license.purchase` (owner/director), `license.activate`/`license.purchase` вне `WRITE_ACTIONS`; waitlist/Stripe-хвост с лицензии и activate-key; редирект `suspended` → `/license-required` (recovery `/settings/license`); копирайт `LicenseRequiredPage`; RLS §8.51 (SELECT org всем members при suspended, license/subscription — owner+director). Monthly-витрина — S3b.
2026-09-11 — fix(security): микропатч **2.11.4** (S2d) — `REVOKE INSERT` на `platform_purchase_requests` для `authenticated`, снята INSERT policy; `platform_purchase_requests_kind_guard` блокирует любой JWT INSERT; заявки только через Edge/RPC `submit_platform_purchase_request`; SQL-тест `crm_monthly_subscription_s2d_test.sql`; R6-тест обновлён.
2026-09-11 — feat(billing): микропатч **2.11.3** (S2c) — Dev Console Inbox: `activate_platform_purchase_request` (month/lifetime, идемпотентный 200, preview периода), фильтр kind; Billing: RPC `dev_console_adjust_organization_subscription` (create-if-missing, extend, provider manual|stripe, audit before/after); Edge `dev-console-purchase-inbox` / `dev-console-adjust-subscription`; SQL-тест `crm_monthly_subscription_s2c_test.sql`. RLS INSERT — S2d.
2026-09-11 — feat(billing): микропатч **2.11.2** (S2b) — Edge `create-purchase-quote` (JWT, `resolvePaymentQuote`, purge window) и `submit-purchase-request` через RPC `submit_platform_purchase_request` (`quote_id` + `client_request_id`, fail-closed без quote/суммы с клиента); CRM lifetime: выбор способа + quote перед submit; email заявок inline; `npm run test:edge:purchase-quote`. Inbox month — S2c.
2026-09-11 — feat(billing): микропатч **2.11.1** (S2a) — `platform_purchase_quotes`, kind `crm_subscription`, `purchase_review_hold_until`, `billing_anchor_day` + `add_calendar_month`, RPC `submit_platform_purchase_request` / `activate_platform_purchase_request`, патч `purge_expired_demo_organizations` и `organization_has_active_subscription` (manual без null end); SQL-тест `crm_monthly_subscription_s2a_test.sql`. Edge/Inbox UI и RLS INSERT — S2b–S2d.
2026-09-11 — feat(billing): подверсия **2.11.0** (S1) — контракт `platform_payment_methods.config` schemaVersion 2: канон `crmLifetime`/`crmMonthly`, override по способам, stable crypto `id`, `resolvePaymentQuote` (CRM + Edge `_shared/paymentQuote.ts`), Dev Console `/payment-methods` (секции CRM, месячные поля, compare-and-swap `pricingRevision`), `npm run test:platform-payment-contract`. Продажа месяца в CRM UI ещё не включена.
2026-09-11 — docs(billing): сверка `.cursor/docs/ai/crm_monthly_subscription_and_support_bot.md` с кодом **2.10.39** — RLS `suspended` только owner (§8.51), «S9»≠аудит S09, S2b обязан выбрать способ для quote, T−7 только S4, digest enqueue в S5b; последовательность промптов с чекбоксами в §16. Код не менялся.
2026-09-11 — fix(ui): микропатч **2.10.39** — `QueryErrorState` больше не показывает сырые ключи вроде `renters.error.notFound`; карточка арендатора при отсутствии записи — «Арендатор не найден» и кнопка «К списку», а не общее «Не удалось загрузить данные».
2026-09-11 — fix(schedule): микропатч **2.10.38** — правка группового урока с датой окончания больше не «успешна» без записи: серия с `valid_to` не считалась живой, UPDATE с `valid_to IS NULL` не трогал строки, новая версия ставилась на сегодня вместо даты занятия. Версия с даты открытого урока, конец серии копируется, toast только если строки реально обновлены.
2026-09-11 — fix(hall-rent): микропатч **2.10.37** — «Резерв 50%» / «Доступно» после T−24 ч: вторая половина `prepaid_charged` слота больше не выглядит свободной и не уходит в FIFO. Пакет 3 дня × 50% показывает резерв на все дни, пока remainder не списан; remainder в `time_end` по-прежнему не ест чужой `active`-резерв. Миграция `20261113000001`.
2026-09-11 — feat(hall-rent): микропатч **2.10.36** / Mini App **0.1.14** — бухгалтер, директор и владелец (и админ с флагом баланса) могут исправить сумму Mini App-кошелька на карточке арендатора: RPC `preview_staff_renter_wallet_adjust` / `staff_renter_wallet_adjust`, ledger `wallet_correction_credit`/`wallet_correction_debit`, причина + кто + когда. Пол не ниже obligated (долг + холды + remainder). Дебет уменьшает `rental_advances`, кредит создаёт корректирующий аванс, затем `_renter_apply_wallet`. Миграция `20261112000001`.
2026-09-11 — fix(hall-rent): Mini App **0.1.13** — сумма к оплате в кабинете арендатора: 100% стоимости аренды + долг (не 50% предоплаты). В расчёте брони отдельная строка долга и «К оплате»; CTA пополнения то же число. Активация на сервере по-прежнему резервирует 50%, остаток списывается после начала слота.
2026-09-11 — docs(billing): ревью спеки `.cursor/docs/ai/crm_monthly_subscription_and_support_bot.md` — снимок **2.10.39**, §5.8 чеклист платформенного Telegram-бота, bootstrap Group Privacy/webhook, пробелы `isReadOnly` (suspended / истёкший period), landing RU/EN, inline email → outbox; код не менялся.
2026-09-11 — docs(billing): спека месячной подписки CRM и платформенного Telegram-бота поддержки — `.cursor/docs/ai/crm_monthly_subscription_and_support_bot.md` (ручной рельс как lifetime, Inbox, цена в Dev Console, тикеты; код не менялся).
2026-09-11 — fix(schedule): микропатч **2.10.35** — `tsc`: сужение `priceId` в `EditLessonPopup`, i18n `common.loading.data` в оплате, импорт `dowShort` в блоке «без преподавателя».
2026-09-11 — fix(schedule): микропатч **2.10.34** — оплата персонального урока из попапа расписания: не ждать отдельный `usePersonalLessons` (при первом клике «Оплатить» запрос ещё пустой → ложная «Не удалось загрузить данные урока»); данные берутся из карточки расписания, при наличии — из полной записи урока.
2026-09-11 — feat(hall-rent): микропатч **2.10.33** — в настройках организации флаг «Администратор может изменять баланс арендаторов» (`admin_can_manage_renter_balance`): вкладка Финансы и Mini App-кошелёк на карточке, без полного `renters.finance.read`. Приём оплаты за уроки/аренду по-прежнему `admin_can_accept_payments`. Миграция `20261111000001`.
2026-09-11 — fix(hall-rent): Mini App **0.1.12** — убраны баннеры про Start/`/start` и недоставленные Telegram-уведомления (бот арендатору не пишет).
2026-09-10 — fix(hall-rent): микропатч **2.10.32** / Mini App **0.1.11** — постоянная аренда на iPhone: WKWebView мог отдать локализованный текст `<select>` вместо `YYYY-MM-DD`, Postgres падал на `::date` → «Ошибка сервера». Клиент держит ISO через selectedIndex, weekday без Intl-имён; RPC нормализует дату/weekdays и не отдаёт 500. «Дата первого занятия» в формате `03 июля 2026`. Миграция `20261110000008`.
2026-09-10 — feat(hall-rent): Mini App **0.1.10** — сетка расписания визуально как CRM: колонки дней, блоки по длительности (pastel «Занято» / slate «Моя бронь»), Inter, контролы h-8; логика occupancy/брони без изменений.
2026-09-10 — fix(personal): микропатч **2.10.31** — бронь «С оплатой» из расписания: не инвалидировать кэш до завершения `record_personal_lesson_payment` (устранён race, когда refetch показывал неоплаченные уроки); оплата по `personal_lesson_charges` с `p_charge_id`; при равном делении — закрываются все начисления; idempotency per charge; RPC учитывает `payer_client_id`. Миграция `20261110000007`.
2026-09-10 — fix(hall-rent): микропатч **2.10.30** — в настройках Mini App простым языком: какую ссылку вставлять в чат уведомлений (`https://t.me/…` / `t.me/+`), что web.telegram.org не подходит, как взять приглашение закрытой группы и зачем передобавить бота.
2026-09-10 — feat(hall-rent): микропатч **2.10.29** — чат для чеков Mini App и чат для уведомлений CRM разделены (`telegram_staff_alert_chat_url`). Арендатор шлёт скрин по своей ссылке; бот дублирует алерт в группу или личку по выбору владельца. Смена ссылки чеков не сбрасывает привязку алертов. Миграция `20261110000006`.
2026-09-09 — fix(hall-rent): микропатч **2.10.28** — очередь перерасчёта после «Отменить пакет»: per-slot close больше не съедает `bulk_pack`/`ban`; review и при уже `cancelled` серии; только первая календарная неделя пакета; бан откладывает FIFO/close как пакет. UI: сумма без дубля валюты.
2026-09-09 — fix(hall-rent): микропатч **2.10.27** — перерасчёт пакета: удалены legacy-перегрузки `_renter_early_close_pack(uuid)` / `_renter_after_pack_slot_terminal(uuid)` (иначе поштучная отмена снова автосписывала surcharge); очередь только при `used_week_count = 1`; grants service_role; CRM-панель — права и ошибка загрузки.
2026-09-09 — feat(hall-rent): микропатч **2.10.26** — досрочное закрытие пакета Mini App: автоматический перерасчёт по разовому тарифу убран; очередь `rental_series_surcharge_reviews` при массовой отмене после 1-й недели; персонал (owner/director/admin/accountant) применяет или отменяет перерасчёт; при 2+ неделях использования или поштучной отмене — без очереди. RPC `apply_renter_pack_surcharge` / `waive_renter_pack_surcharge`; уведомление в кабинете арендатора; блок в карточке арендатора CRM. Миграция `20261110000003`; `rental_rules_ru.md`.
2026-09-09 — fix(hall-rent): микропатч **2.10.25** — чат для чеков: invite-группа не привязывается первой попавшейся (кандидат + подтверждение в CRM); `/start` в личке оживляет отложенные staff-алерты; в настройках Mini App — шаг и статус про команду `/start`. Миграция `20261110000002`.
2026-09-09 — feat(hall-rent): микропатч **2.10.24** — HALL-RENT-TOPUP-2: бот студии дублирует короткий алерт о заявке Mini App в чат для чеков (`staff_topup_submitted` в `renter_telegram_outbox`, group chat_id < 0). Заявка не падает, если Telegram недоступен. На карточке арендатора — `@username` + кнопка «Написать в Telegram» (owner/director/admin/accountant). Статус привязки чата чеков в настройках Mini App. Миграция `20261110000001`.
2026-09-09 — fix(hall-rent): Mini App **0.1.8** — панель зала/недели/легенды в «Расписании» сворачивается при прокрутке сетки и разворачивается по кнопке ▼.
2026-09-09 — fix(hall-rent): Mini App **0.1.7** — форма брони не закрывается при «Пополнить» (sheet на уровне кабинета); sticky-шапка дат в сетке расписания; сохранение QR — индикатор «Сохранение…» и подсказка об ожидании на iPhone.
2026-09-09 — fix(schedule): микропатч **2.10.23** — попап добавления группового/персонального урока не закрывается при создании дисциплины (initial loading без refetch, Escape/nested modal, без сброса формы при обновлении списка дисциплин).
2026-09-09 — fix(ui): микропатч **2.10.22** — всплывающие ошибки и конфликты на человеческом языке: `resolveMutationError` маскирует технические RPC-коды; завершение/переоткрытие урока, посещаемость и оплаты без сырого `snake_case`; уточнены тексты конфликтов расписания, серий аренд, офлайн-сверки.
2026-09-09 — feat(schedule): микропатч **2.10.21** — редактирование персонального урока: чекбокс «Изменить все будущие записи» применяет изменения ко всей еженедельной серии (клиенты, время, преподаватель, дисциплина, тариф); дата меняется только у текущего урока.
2026-09-09 — fix(schedule): микропатч **2.10.20** — «могильники» удаления больше не занимают дату: retire = `valid_to = valid_from - 1` (однодневный урок остаётся `valid_to = valid_from`); зачистка существующих tombstone; overlap/personal/expand игнорят retired. Миграция `20261109000007`.
2026-09-09 — fix(schedule): микропатч **2.10.19** — добавление группового занятия: ошибка пересечения на человеческом языке (дата/время первого конфликта + общее число); проверка по `valid_from`…`valid_to` как в БД; retired `schedule_slots` больше не блокируют новый слот (`prevent_schedule_slot_overlap`). Миграция `20261109000006`.
2026-09-09 — fix(ui): микропатч **2.10.18** — `DatePickerField`: календарь закреплён по верхней границе при смене месяца (фиксированная сетка 6 строк, без пересчёта top по высоте); горизонтальное позиционирование не выходит за края экрана на мобильных.
2026-09-09 — fix(hall-rent): Mini App **0.1.6** — в «Правилах» убраны вводный абзац и префикс «Самое главное» в блоке дисклеймера.
2026-09-09 — fix(hall-rent): Mini App **0.1.5** — версия в футере кабинета; убрана верхняя кнопка «Постоянная аренда» в «Расписании» (пакет по-прежнему через «Постоянная» в форме брони ячейки).
2026-09-08 — fix(hall-rent): микропатч **2.10.17** / Mini App **0.1.4** — отменённые брони Mini App исчезают из сетки и «Мои записи»: `renter_get_occupancy.mine` и `renter_list_mine` без `cancelled`/`hold_deleted`/`auto_deleted`; occupancy.busy не показывает retired-слоты и отменённые групповые occurrence. Клиент `occupiesMiniAppGrid`. Миграция `20261109000005`.
2026-09-08 — feat(hall-rent): Mini App **0.1.3** — кнопка «Правила» в шапке кабинета; полноэкранный раздел с текстом `docs/rental_rules_ru.md` (импорт markdown в бандл).
2026-09-08 — fix(hall-rent): микропатч **2.10.16** — `create_rental_series` больше не падает после чистого preview: `schedule_location_has_conflict` выровнен с `preview_rental_conflicts` (retired `schedule_slots`, отменённые occurrence). Миграция `20261109000004`; SQL-тест `schedule_location_has_conflict_retired_test.sql`. i18n: `schedule.rental.conflict`.
2026-09-08 — fix(hall-rent): микропатч **2.10.15** — предпросмотр серии аренд: число слотов выводится напрямую (`9 слотов`), без i18n-плейсхолдера `{count}`.
2026-09-08 — fix(hall-rent): микропатч **2.10.14** — серия аренд больше не конфликтует с удалёнными групповыми слотами: retired `schedule_slots` (`valid_to <= valid_from`) и отменённые occurrence исключены из `preview_rental_conflicts`, `_renter_location_slot_busy`, `preview_calendar_event_conflicts`; клиентский `findScheduleConflict` — `isRetiredScheduleSlot`. Миграция `20261109000003`. i18n: `rentalSeries.occurrenceCount` — `{{count}}`.
2026-09-08 — feat(prices): микропатч **2.10.13** — тарифы аренды зала перенесены из «Настройки → Аренда зала» в подраздел «Аренда зала» на странице «Тарифы и прайс-лист» (`/prices?section=hall-rent`); бухгалтер и админ с lookup получают доступ к прайс-листу только для этого подраздела.
2026-09-08 — fix(schedule): микропатч **2.10.12** — удаление группового занятия снова закрывает слот с даты (а не «отмену» без обновления сетки); отменённые occurrence скрываются в `expandSlotsToWeek`; `closeScheduleSlot` проверяет, что строка обновлена; `useDeleteGroupSchedule` закрывает слоты с конечным `valid_to`.
2026-09-08 — fix(schedule): микропатч **2.10.11** — удаление группового занятия с первой даты серии больше не падает на `schedule_slots_check` (retire: `valid_to = valid_from`); в попапе восстановлена кнопка «Удалить все в будущем» для повторяющихся групп и персональных серий (загрузка будущих дат); для мульти-дневных групп — удаление всех дней с даты.
2026-09-08 — fix(crm): микропатч **2.10.10** — «Обзор и статистика» (teacher scoped): «Сегодня в расписании» через `expandSlotsToDateRange` (учёт valid_from/valid_to и переносов, без устаревших слотов по day_of_week); персональные уроки — `maskClientDisplay` вместо сырого `schedule.lessonInfo.clientNotSpecified`, фильтр по преподавателю.
2026-09-08 — fix(crm): микропатч **2.10.9** — преподавателем урока могут быть только владелец, директор и преподаватель: общий фильтр `lessonTeacherRoles`, триггеры на `schedule_slots` / `personal_lessons` / `single_visits`, RPC замены преподавателя.
2026-09-08 — fix(crm): микропатч **2.10.8** — «Обзор и статистика»: операционный дашборд грузит только неоплаченные персональные уроки (`paidFilter: "no"`), а не всю историю (таймаут на больших студиях); teacher scoped — только будущие уроки; PostgREST-ошибки показывают `message`, не `[object Object]` (`queryError.ts`).
2026-09-08 — fix(dev-console): микропатч **2.10.7** — `isDeveloperVerified` в Edge Functions: fallback на `is_platform_developer` RPC (актуальный `auth.users`, не только JWT); Dev Console показывает signed-in email и экран «Developer access required» до загрузки Tenants/Keys.
2026-09-08 — fix(crm): микропатч **2.10.6** — multi-CRM login больше не открывает старый `user_active_organizations` без выбора: после email-входа пользователь с несколькими membership попадает на `/select-organization`, `OrgSwitcher` виден в верхней панели и показывает роль; `authStorage` без remember-me больше не читает старую localStorage-сессию; Dev Console объясняет `developer_access_required`. Production: у `albertkoall@gmail.com` сброшен active org row (`Miami Studio` admin), developer `platform_role` уже был задан.
2026-09-08 — feat(hall-rent): микропатч **2.10.5** — постоянная аренда Mini App и CRM: выбор пакета на 2, 3 или 4 недели (раньше только 4). Миграция `20261108000001`, `recurringPackWeeks`, UI в `BookingSheet`, `PackSheet`, `CreateMiniAppBookingDialog`.
2026-09-07 — fix(dev-console): валидация `DEV_CONSOLE_ISSUER_SIGNATURE` — plaintext или pre-computed HMAC (64 hex), NFC-нормализация; скрипт `hash-issuer-signature.mjs`; секрет перевыпущен.
2026-09-07 — fix(ops): микропатч **2.10.4** — purge org: отключение `%calendar_sync%` триггеров перед `DELETE FROM organizations` (FK `calendar_sync_outbox_organization_id_fkey`); удалена тестовая база Test studio 2 (`29c7e55b-…`). Миграция `20261107000001`.
2026-09-07 — fix(ops): микропатч **2.10.3** — все `test:db:*` идут через `psql-local-test.mjs` (отказ от hosted `DATABASE_URL`); убран silent fallback на linked pooler; `hall-rent-integration-check` и `db:reset-test` больше не бьют `--linked` production без `ALLOW_PROD_DB_RESET=1`.
2026-09-06 — fix(ops): с production сняты 5 leftover Mini App concurrent-фикстур с ложным Lifetime (`FA3/FA7/FDB4 * Org`, без ключа покупки); helper-RPC и `*@test.local` пользователи удалены. Concurrent-тесты чистят фикстуры в `finally`, отказываются от hosted `DATABASE_URL` без `ALLOW_PROD_DB_TESTS=1`.
