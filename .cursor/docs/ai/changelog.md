2026-08-28 — fix(security): микропатч **2.8.117** — S38 / L2+L3+L4+L5+L9+L12+L17+L19+L20: `ACTIVATION_DEBUG=false` в `.env.example`; SRI на Telegram Web App SDK; GCal webhook сверяет `resource_id`+`channel_token`; cron только `x-cron-secret` (constant-time, не `Authorization`); recovery JWT не открывает CRM-shell; пустой `ALLOWED_ORIGINS` на hosted → 503 `allowed_origins_not_configured`; `isDeveloper` только `app_metadata` после `getUser`; `refresh_token_reuse_interval = 0`; `extra_search_path = ["public"]` + `SET search_path` на DEFINER без него.
