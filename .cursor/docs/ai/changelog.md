...
2026-06-26 — S7 (Промт 14): security hardening — `ACTIVATION_DEBUG` default `false` (prod без debug в ответах/audit message); `_shared/postgrestSearch.ts` (sanitize `%_\\,().`, max 100) для Dev Console EF: `dev-console-search-orgs`, `dev-console-search-billing`, `dev-console-list-errors`, `dev-console-list-tenants`; `.env.example` — `ACTIVATION_DEBUG=true` только для local/staging.
