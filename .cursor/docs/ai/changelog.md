2026-08-26 — fix(finance): микропатч **2.8.73** — RPC `write_off_personal_lesson_debt` / `get_personal_lesson_debt_trace`: кнопка «Удалить» в корректировке дебиторки больше не падает на schema cache; начисление приводится к уже принятой оплате, платежи не трогаются.
