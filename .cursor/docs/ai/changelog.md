2026-09-04 — fix(personal): микропатч **2.9.72** — запись пары с «Повторять еженедельно»: `applyLessonTariff` больше не удаляет уже выбранного второго клиента при solo-тарифе (типичный сценарий — клиенты выбраны до клика «Разовый урок»); тип урока и `client_id2..4` считаются по заполненным полям, не по числу слотов в форме. Общие хелперы в `personalLessonClients.ts`.
