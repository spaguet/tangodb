...
- **Как избежать:** после кассового create персонала, если у арендатора есть `telegram_id` и `available >= 50%`, промотировать слот в Mini App и вызвать `_renter_apply_wallet`. Не промотировать слоты с кассовой оплатой, счетами или нехваткой баланса (иначе холд сгорит за 24 ч).
