...
- **Ошибка:** оплата урока 28.08 требовала подтверждения, правило действовало до 31.08.
- **Причина:** гейт смотрел на `current_date` (день оплаты). Диалог брал статус «на сегодня». Telegram кэшировал старый JS.
- **Как избежать:** `venue_cost_status_for_org(org, lesson_date)`; не подменять `acknowledgement_required` статусом на сегодня; если дата услуги ≤ `latest_valid_to` — не требовать ack; HTML `Cache-Control: no-store`.
