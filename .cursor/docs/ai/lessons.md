...
- **Причина:** невидимый `fixed inset-0` backdrop (OrgSwitcher / меню аккаунта) перехватывал клики; `mousedown` у датапикера/групп глотал `click` по ссылке; `AppLayout` делал `el.remove()` чужих React-порталов и мог заморозить оболочку. React Router 7 перехватывает `<a>` — без синхронного закрытия слоёв переход «зависал» на текущей странице.
- **Как избежать:** не класть полноэкранный прозрачный backdrop под шапочные меню (click-outside по `click`); закрывать оверлеи событием `tangodb:dismiss-overlays` до навигации; не вырезать порталы из DOM вручную; модалки с `fixed inset-0` — `createPortal(document.body)`.
