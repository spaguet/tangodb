...
### 2026-08-17 — Atelier: gold-600 и ink-400 не проходят WCAG AA на светлом фоне

- **Дата:** 2026-08-17
- **Ошибка:** после миграции палитры primary CTA (`bg-gold-600 text-white`) давал 4.0:1 (ниже AA 4.5:1); метки полей `text-ink-400` на white — 3.78:1; блоки расписания `gold-500`/`lavender-400` + белый текст — ниже порога.
- **Причина:** hex из бренд-спеки рассчитаны на filled-кнопки и декоративные блоки; WCAG для **текста** на светлом требует более тёмные ступени (`gold-700` для CTA, `ink-500` для меток).
- **Как избежать:** на светлом CRM — filled CTA только `bg-gold-700`+`text-white`; inline-ссылки — `text-gold-700` (уже в §5); метки форм — `ink-500`, не `ink-400`; расписание: групповой — `text-ink-900` на `gold-500` или `gold-700` bg; персональный — `lavender-500` bg минимум. На тёмном dev-console `ink-400` на `ink-900` проходит AA — не менять на `ink-500`.
