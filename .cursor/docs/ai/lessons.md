...
### 2026-09-18 — После клика «Прайс-лист» на кассе не работали кнопки

- **Ошибка:** ссылка «Прайс-лист» на `/subscriptions/sell` не открывала «Тарифы и прайс-лист», после клика переставали нажиматься остальные кнопки, включая левое меню. Повторилось после 2.11.43–2.11.46.
- **Причина:** React Router 7 `BrowserRouter` по умолчанию коммитит смену маршрута через `React.startTransition`. History/URL обновляются сразу, а React-состояние — с низким приоритетом. На тяжёлых экранах (касса абонемента, дашборд) непрерывные обновления TanStack Query не давали переходу закоммититься: URL уже `/prices` или `/attendance`, `Outlet` оставался на старой странице. Overlay-факторы 2.11.45 тоже опасны, но сами по себе не объясняли рассинхрон URL и UI. Жёсткий `window.location.assign` в 2.11.46 уводил с кассы, но SPA-меню после этого оставалось на том же механизме и снова «замирало».
- **Как избежать:** для CRM-оболочки с живыми Query-обновлениями ставить `<BrowserRouter useTransitions={false}>`, чтобы `Outlet` менялся синхронно с URL. Не чинить навигацию точечным `location.assign`. Не класть прозрачные full-screen backdrop под шапочные меню; не вырезать React-порталы из DOM вручную.

### 2026-09-16 — Оплата счёта: «Could not choose the best candidate function»

- **Ошибка:** при оплате счёта арендатора тост с текстом Postgres о выборе между двумя `record_rental_invoice_payment`.
- **Причина:** миграция fiscal добавила 10-arg overload, но 5-arg из `rental_invoices_advances_ui` остался; вызов с `p_operation_date` неоднозначен.
- **Как избежать:** после расширения RPC fiscal-полями — отдельная миграция `DROP FUNCTION` legacy overload (как `20260871000001` для `record_rental_payment`).
...
- **Причина:** ближайший слот попал в окно T−24: FIFO/`expire_and_catchup` сразу `prepaid_charged` (списание 125000) и earmark остатка ещё 125000. Второй слот держал 125000 в резерве. Правило T−24 нужно самообслуживанию, не CRM-брони персонала.
- **Как избежать:** staff (`created_by IS NOT NULL`) активировать как `active` 50% до `time_start`; T−24 charge только при `created_by IS NULL`.

### 2026-09-14 — Staff Mini App-бронь без долга Mini App и занятый слот после удаления холда

- **Ошибка:** админ бронировал через кнопку Mini App — слот есть, «Долг Mini App» = 0, та же сумма в кассовом «не выставлено в счёт». После удаления холда «Аренда» на то же время давала конфликт занятости.
- **Причина:** `renter_create_booking` ставит `awaiting_payment`/`debt_amount=0`, а не `lifecycle=debt`. `get_renter_rental_finance` считал Mini App кассой. Occupancy смотрела только `booking_status = confirmed` и не отбрасывала `hold_deleted`. 2.11.18 не промотила короткий кошелёк в Mini App.
- **Как избежать:** staff-слот с Telegram → Mini App; нет 50% на холд → долг на полную стоимость; occupancy через `_rental_occupies_hall`; кассовый долг только `channel = cashier`. В CRM не держать отдельную кнопку Mini App рядом с «Аренда».

### 2026-09-13 — Mini App: две кнопки пополнения и потерянный жест Telegram

- **Ошибка:** арендатор слал чек без заявки в CRM или заявку без чека; после submit чат мог не открыться, а буфер — пустой.
- **Причина:** две параллельные CTA; `await` refresh кошелька до `openTelegramLink` / clipboard съедал user-gesture; ошибка handoff попадала в тот же `catch`, что сбой RPC; в чат уходила клиентская сумма, не `result.amount`.
- **Как избежать:** одна CTA = pending + код, затем handoff (clipboard не блокирует open); refresh кошелька после чата; черновик показывать на экране; ошибку чата не выдавать как «заявка не создана».

### 2026-09-13 — Админ создал аренду, кошелёк не удержал 50%

- **Ошибка:** на карточке арендатора кошелёк полный, «Резерв 50%» = 0, вся сумма висит кассовым долгом; после слота остаток тоже не списывается.
- **Причина:** `CreateRentalDialog` / `create_rental` пишет `channel = cashier`. FIFO, T−24 и remainder worker работают только для `channel = miniapp`. Админ выбирал первую кнопку «Аренда» (касса), а не Mini App.
- **Как избежать:** после кассового create персонала, если у арендатора есть `telegram_id`, промотировать слот в Mini App и вызвать `_renter_apply_wallet`. Если `available < 50%` — `lifecycle = debt` на полную стоимость, а не 24ч-холд и не кассовый uninvoiced. Не промотировать слоты с кассовой оплатой или счетами.

### 2026-09-12 — Персональный урок на двух клиентов сохраняет только первого

- **Ошибка:** при создании персонального урока с двумя клиентами второй клиент мог исчезнуть до сохранения; в карточке затем отображался только первый.
- **Причина:** выбор способа оплаты/тарифа вызывал `applyLessonTariff`, которая безусловно подгоняла количество клиентских строк под тип тарифа через `pop()`. Если оператор уже выбрал второго клиента, эта строка удалялась из состояния формы перед `useAddPersonalLessons`.
- **Как избежать:** автоподгонка тарифа может добавлять недостающие поля, но не должна удалять заполненные клиентские строки. Если нужно убрать лишних участников, требовать явного действия оператора.

### 2026-09-11 — Карточка арендатора: «Не удалось загрузить данные» + `renters.error.notFound`

- **Ошибка:** RPC `get_renter_detail` вернул `renters.error.notFound`, хук бросил `Error` с этим ключом, `QueryErrorState` вывел generic «Не удалось загрузить данные» и сырой ключ без перевода.
- **Причина:** ключ уже есть в i18n («Арендатор не найден»), но экран ошибки загрузки не вызывал `t()` / `resolveMutationError` (в отличие от toast-мутаций); `!data` на карточке смешивался с сбоем запроса.
- **Как избежать:** в `QueryErrorState` переводить dotted i18n keys в заголовок; notFound не считать падением загрузки — пустой результат + «К списку».

### 2026-09-11 — Расписание: «сохранено», но групповой урок не меняется

- **Ошибка:** правки Silver (и др.) с окончанием «до даты» показывали успех, в сетке оставалась старая версия; подсказка «с 11.09» при занятии 16.09.
- **Причина:** живой слот с `valid_to` (конец серии) обрабатывался как уже закрытый; metadata UPDATE фильтровал `valid_to IS NULL` и не проверял число строк; `editDate` брался как сегодня, а не дата открытого урока.
- **Как избежать:** live = `isScheduleSlotLiveOnDate` (не `valid_to IS NULL`); версия с `lesson.date` с копированием `valid_to`; после UPDATE — `.select("id")` и ошибка при 0 строк.

### 2026-09-11 — Mini App: резерв пакета 2 дня вместо 3 после T−24 ч

- **Ошибка:** Uliana оплатила пакет на 11/18/25 сентября; карточка показывала «Доступно 100000», «Резерв 50% 200000» вместо резерва на все три дня.
- **Причина:** `_renter_wallet_reserved_prepay` считал только `lifecycle=active` без charge. 11 сентября уже `prepaid_charged` (окно T−24 ч), поэтому её 50% ушли из резерва, а неспаренный remainder того же слота оставался в spendable/FIFO.
- **Как избежать:** UI-резерв = uncharged active prepay + remainder `prepaid_charged`; spendable/available не тратят earmark; remainder charge берёт `wallet − reserved_active`, не чужой active-резерв. Инвариант только на `reserved_active`.

### 2026-09-11 — Расписание: «Не удалось загрузить данные урока» при первой оплате

- **Ошибка:** кнопка «Оплатить» в попапе урока сразу показывала toast, хотя карточка уже отображала клиента и долг.
- **Причина:** `handleOpenPay` требовал строку из отдельного `usePersonalLessons`, который стартует только при открытии попапа; до завершения запроса `data` пустой.
- **Как избежать:** для оплаты использовать уже загруженный `PersonalDisplayLesson` из сетки расписания; полный список уроков — только для серий/удаления, не как hard gate для pay.
...
- **Причина:** в 0.1.7 sheet брони оставили открытым (правильно), но `openTopup` по-прежнему делал `setTab("mine")`.
- **Как избежать:** CTA из модали не менять нижний слой навигации; открывать вложенный sheet поверх той же формы.
...
- **Причина:** удалённый групповой слот retire-ится как `valid_to = valid_from`; `expandSlotsToWeek` его скрывает, а `preview_rental_conflicts` всё ещё считал дату в диапазоне `valid_from..valid_to`.
- **Как избежать:** occupancy-проверки (аренда, mini app, события) использовать `_schedule_slot_active_on_date` / `isRetiredScheduleSlot`; учитывать `schedule_occurrence_cancellations` так же, как сетка расписания.
...
- **Причина:** `parseInviteTokenFromLocation` брал boot-stash **раньше** hash; при смене `#token=` без полной перезагрузки (или после первого отозванного инвайта) в stash оставался старый `TDB-INV-…`.
- **Как избежать:** приоритет URL (query/hash) над stash; stash — только fallback после wipe Telegram; на `hashchange` сбрасывать preview.

### 2026-09-05 — Инвайт преподавателя: «недействительно или истекло»

- **Ошибка:** Ссылка `/accept-invite#token=TDB-INV-…` показывала «Приглашение недействительно или истекло», хотя инвайт был свежий.
- **Причина:** `telegram-web-app.js` грузится на всех страницах CRM и переписывает `location.hash` до React — токен из fragment терялся. Парсер брал весь хвост после `#token=` и ломался, если Telegram дописывал свои параметры. При уже открытой сессии CRM (владелец проверяет ссылку) `preview` пропускался, `accept-invite` падал на чужом email и маскировался под generic/expired.
- **Как избежать:** снимать `#token=` синхронным `/invite-token-boot.js` до Telegram SDK; парсить hash через `URLSearchParams`; всегда делать `preview-invite` до accept; mismatch email → «выйти и продолжить», не «истекло».
...
- **Причина:** `html-to-image` не рисует цвета Tailwind v4 (`oklch`) и копирует off-screen `left: -10000px` в SVG-клон — в кадре ничего нет.
- **Как избежать:** не снимать DOM CRM через `html-to-image`/`html2canvas`; для расписания рисовать сетку на canvas по данным (`exportSchedulePng`).

### 2026-09-05 — Mini App: два аккаунта → один профиль

- **Ошибка:** Вход с другого Telegram-аккаунта показывал профиль первого пользователя.
- **Причина:** `App.tsx` при наличии persisted GoTrue-сессии в `localStorage` (ключ `tangodb-renter-auth-{orgId}`) пропускал `mintRenterSession` и сразу вызывал `renter_bootstrap` — сессия привязана к первому `telegram_id`.
- **Как избежать:** перед reuse сессии сверять `app_metadata.telegram_id` с `user.id` из текущего `initData`; при расхождении — `signOut` и полный mint (сервер создаёт/находит renter по `telegram_id`).

### 2026-09-04 — CRM: две «Подписи» и ошибка удаления QR

- **Ошибка:** В настройках аренды два QR с одинаковой подписью «Подпись (банк, кошелёк)»; удаление → `renter.qr.deleteFailed`.
- **Причина:** Пустой `label` в списке подставлял i18n-ключ поля формы; `DELETE` из `organization_rental_qr_assets` падал на FK `renter_topup_requests.qr_asset_id` для resolved заявок.
- **Как избежать:** для пустого label — дата загрузки, не текст label поля; перед удалением QR отвязать `qr_asset_id` у confirmed/rejected; pending по-прежнему блокирует delete.
...
- **Причина:** клиент Mini App жил на короткоживущем signed URL без принудительного перевыпуска при повторном показе/скачивании. Параллельно Mini App-topup уже была починена без `notes`, но отдельные CRM RPC/JSON всё ещё читали и писали несуществующую колонку `rental_advances.notes`.
- **Как избежать:** для приватных QR выдавать свежий signed URL по user action через Edge/service-role, а не рассчитывать, что старый URL ещё жив. При follow-up по схеме аренды проверять все RPC и представления, которые трогают `rental_advances`, а не только один путь topup.
...
- **Причина:** под полями стоял allowlist URL и путь BotFather, без «зачем» и без «своё приложение писать не нужно».
- **Как избежать:** в полях — назначение (чат для чеков, короткое имя в боте); пошаговый путь — под спойлером.

### 2026-09-02 — правило зала «истекло», хотя урок внутри срока

- **Ошибка:** оплата урока 28.08 требовала подтверждения, правило действовало до 31.08. Правки 2.9.21–2.9.24 не проявлялись в UI.
- **Причина:** SQL на проде уже сравнивал **дату урока** (`ack` для 28.08 = false). Фронт на Vercel остался на **2.9.20**: деплой 2.9.24 упал (`filterNewPersonalSeriesSlots` импортируется, но не экспортирован). Старый клиент вызывает `get_venue_cost_rule_status` **без** даты урока → сравнение с сегодня.
- **Как избежать:** после push проверять статус Vercel (не только git); `npm run build` на том же дереве, что уходит в коммит; гейт оплаты — дата записи, не `current_date`.


### 2026-09-02 — пара в CRM, один человек в Google Calendar

- **Ошибка:** в сетке «Вероника & Аня», в Google Calendar только первый человек.
- **Причина:** заголовок события склеивал имена через запятую (недельный вид Google показывает первый токен); после 2.8.15 имена убрали из описания; `&` в HTML-описании Google съедает хвост; reconcile не трогает уже synced события.
- **Как избежать:** в Google-заголовке и описании соединять людей через « и »; дублировать список в `Клиенты:`; при смене формата ставить outbox `upsert` по будущим урокам, не `refresh_member`.

### 2026-09-02 — пересечение при правке персоналки (клиент / weekly / GCal)

- **Ошибка:** добавление второго клиента в существующий слот блокировалось пустым плательщиком; при «повторять еженедельно» сохранение падало с неясным пересечением; freebusy предупреждал о занятости Google на этом же уроке.
- **Причина:** эффект плательщика обнулял значение вместо первого клиента; weekly на edit пытался INSERT уже существующие даты, а конфликт проверялся только по текущей неделе; freebusy не исключал слот, который уже синхронизирован в Google.
- **Как избежать:** при 2+ клиентах дефолт плательщика = первый клиент; новые даты серии — только свободные слоты, с refetch диапазона; toast конфликта с ФИО и интервалом; freebusy не проверять неизменённый текущий слот.
...
- **Причина:** Аккаунт создаётся только через `complete-invite` (accept-invite); до этого `resetPasswordForEmail` не шлёт письмо, а UI маскировал любые ошибки под success и не объяснял про инвайт.
- **Как избежать:** После success показывать отдельный экран с подсказкой «сначала примите приглашение»; технические сбои (SITE_URL, сеть, rate limit) — явная ошибка; `console.info` в DEV при успешном запросе.
