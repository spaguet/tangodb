...
- **Ошибка:** ссылка «Прайс-лист» на `/subscriptions/sell` не открывала «Тарифы и прайс-лист», после клика переставали нажиматься остальные кнопки. Повторилось после 2.11.43–2.11.44.
- **Причина:** 2.11.45 убрал часть overlay-причин, но оставил CTA как React Router `Link` с side effects (`dismissAppOverlays` + `flushSync`). На воспроизведении URL становился `/prices`, а React-экран оставался на `/subscriptions/sell`: пользователь видел старую форму и «замороженные» кнопки. Ранее найденные факторы тоже опасны: невидимые `fixed inset-0` backdrop, `mousedown` outside-close, ручное `el.remove()` порталов.
- **Как избежать:** для навигационных CTA, которые должны сначала закрыть локальные слои, не смешивать `<Link>` и side effects. Если SPA-переход из проблемного экрана оставляет старый `Outlet`, использовать `button` с `event.preventDefault()`, синхронным закрытием слоёв и жёстким `window.location.assign('/prices')`; не класть прозрачные full-screen backdrop под шапочные меню; не вырезать React-порталы из DOM вручную.
