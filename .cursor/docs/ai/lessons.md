...
### 2026-09-04 — пара клиентов + «Повторять еженедельно» сохраняла solo

- **Ошибка:** при записи персонального урока с двумя клиентами и еженедельным повтором в журнале оставался один клиент (тип `solo`).
- **Причина:** в форме клиенты идут выше выбора «Разовый урок»; клик по разовому тарифу вызывал `applyLessonTariff`, который для solo-тарифа обрезал массив клиентов до одного поля (`while (next.length > neededFields) next.pop()`), удаляя уже выбранного второго клиента; тип урока считался по `bookingClients.length`, а не по заполненным id.
- **Как избежать:** `syncBookingClientFieldsForTariff` — добавлять пустые слоты под тариф, но не удалять поля с выбранным `id`; тип/`client_id*` — только из `filledBookingClientIds`.
