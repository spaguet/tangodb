...
function ClientCardModal({ client, locale, onClose }: { client: DemoClient; locale: Locale; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      <button type="button" className="fixed inset-0 bg-ink-950/40 backdrop-blur-xs" onClick={onClose} aria-label="Close" />
      <div className="relative w-full max-w-lg bg-white rounded-xl border border-ink-200 shadow-xl max-h-[85vh] overflow-y-auto">
        <div className="flex items-center justify-between px-4 py-3 border-b border-ink-100">
          <h3 className="text-base font-semibold text-ink-800">
            {client.firstName} {client.lastName}
          </h3>
          <button type="button" onClick={onClose} className="p-1.5 text-ink-400 hover:bg-ink-50 rounded-lg cursor-pointer">
            <X className="w-4 h-4" />
          </button>
        </div>
        <div className="p-4 space-y-3 text-sm demo-field-disabled">
          <div>
            ...
            <p className="text-ink-700 mt-0.5">{client.phone}</p>
          </div>
          {client.email && (
            <div>
              <p className={labelCls}>{p.email}</p>
              <p className="text-ink-700 mt-0.5">{client.email}</p>
            </div>
          )}
          {client.telegram && (
            <div>
          ...
          {client.note && (
            <div className="border-t border-ink-100 pt-3">
              ...
              <p className="text-ink-600 text-xs mt-1 bg-ink-50 rounded-lg p-2 border border-ink-100">{client.note}</p>
            </div>
          )}
        </div>
      </div>
...
export function ClientsPanel({ locale }: Props) {
  ...
  return (
    <div id="panel-newClient" className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start">
      <div className="lg:col-span-4 bg-white rounded-xl p-4 border border-ink-200 shadow-xs panel-card-stack demo-field-disabled">
        ...
        <div className="panel-form-stack font-sans space-y-3">
          ...
          <button type="button" disabled className="w-full py-3 bg-gold-600/40 text-white text-xs font-semibold uppercase tracking-widest rounded-lg cursor-not-allowed">
            {p.addSubmit}
          </button>
        </div>
      </div>
      ...
      <div className="lg:col-span-8 flex flex-col">
        ...
        <div className={`bg-white p-4 border border-ink-200 shadow-xs panel-card-stack ${pageTabPanelCls(tab, "active")}`}>
          ...
          <div className="overflow-x-auto min-h-[200px]">
            {tab === "active" ? (
              <table className="w-full font-sans text-ink-700 text-left">
                <thead>
                  <tr className="border-b border-ink-100 text-[10px] uppercase text-ink-400 tracking-wider">
                ...
                <tbody>
                  {demoClients.map((c, i) => (
                    <tr
                      key={c.id}
                      className="border-b border-ink-50 hover:bg-ink-50 transition-colors text-sm group cursor-pointer"
                      onClick={() => setCardClient(c)}
                    >
                      <td className="py-3 pl-2 text-ink-400 text-xs">{i + 1}</td>
                      ...
                      <td className="py-3 text-right pr-2 text-ink-300 text-xs">{p.cardTitle}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <table className="w-full font-sans text-ink-700 text-left">
                <tbody>
                  {archivedClients.map((c) => (
                    <tr key={c.id} className="border-b border-ink-50 text-sm opacity-70">
                      <td className="py-3 pl-2 font-semibold">{c.lastName} {c.firstName}</td>
                      <td className="py-3 text-xs text-ink-500 text-right">
