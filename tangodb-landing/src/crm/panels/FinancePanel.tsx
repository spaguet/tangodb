        ...
        <div className="bg-white rounded-xl border border-slate-200/90 shadow-xs overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-sans text-sm font-semibold text-slate-800">{p.paymentsTitle}</h2>
            </div>
            <div className="relative w-full sm:w-64">
              <Search className="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" />
              <input disabled value="" placeholder={p.search} className="w-full pl-9 pr-3 py-2 text-xs border border-slate-200 rounded-lg bg-slate-50" />
            </div>
          </div>
          <div className="hidden sm:grid sm:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)_minmax(0,1fr)_auto] gap-3 px-3 py-2 bg-slate-50 border-b border-slate-100 text-[10px] uppercase tracking-wider font-semibold text-slate-400">
            <span>{p.client}</span>
            <span>{p.source}</span>
            <span>{p.method}</span>
          ...
          {paymentJournal.map((row) => (
            <div
              ...
              className="grid grid-cols-[1fr_auto] sm:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)_minmax(0,1fr)_auto] gap-2 sm:gap-3 items-center px-3 py-3 border-b border-slate-100 last:border-b-0"
            >
              <div className="min-w-0">
                <p className="text-sm font-semibold text-slate-800 truncate">{row.client}</p>
      ...
      {tab === "expenses" && (
        <div className="bg-white rounded-xl border border-slate-200/90 shadow-xs overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-100 flex items-center gap-2">
            ...
            <h2 className="font-sans text-sm font-semibold text-slate-800">{p.expensesTitle}</h2>
          </div>
          {expenses.map((e) => (
            <div
              key={e.description}
              className="grid grid-cols-[1fr_auto] sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] gap-2 sm:gap-3 items-center px-3 py-3 border-b border-slate-100 last:border-b-0"
            >
              <div>
      ...
      {tab === "payroll" && (
        <div className="bg-white rounded-xl border border-slate-200/90 shadow-xs overflow-hidden">
          ...
          {payrollRows.map((row) => (
            <div
              key={row.name}
              className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 items-center px-3 py-3 border-b border-slate-100 last:border-b-0"
            >
              <div>
