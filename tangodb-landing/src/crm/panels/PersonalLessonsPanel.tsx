  ...
  return (
    <div id="panel-personal" className="panel-page-stack">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-base font-semibold text-ink-800 tracking-tight">{s.personal.title}</h2>
      </div>
      ...
      {tab === "view" ? (
        <div
          className={`bg-white p-4 border border-ink-200 shadow-xs panel-card-stack demo-field-disabled ${pageTabPanelCls("view", "view")}`}
        >
          <div className="bg-white rounded-xl p-4 border border-ink-200 shadow-xs panel-card-stack">
            <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
              <div className="flex flex-wrap bg-ink-100 rounded-lg p-1 text-xs font-semibold gap-1">
                {[s.personal.periodWeek, s.personal.periodMonth, s.personal.periodRange].map((label, i) => (
                  <button
                    ...
                    type="button"
                    disabled
                    className={`px-3 py-1.5 rounded-md ${i === 0 ? "bg-white text-ink-900 shadow-xs" : "text-ink-500"}`}
                  >
                    {label}
                  </button>
                ))}
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-sm font-semibold text-ink-800 min-w-[140px] text-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              <label className="block space-y-1">
                ...
                <div className={fieldCls + " bg-ink-50 text-ink-600"}>{s.personal.allLocations}</div>
              </label>
              <label className="block space-y-1">
                <span className={labelCls}>{ps.discipline}</span>
                <div className={fieldCls + " bg-ink-50 text-ink-600"}>{s.personal.allDisciplines}</div>
              </label>
              <label className="block space-y-1">
                <span className={labelCls}>{ps.teacher}</span>
                <div className={fieldCls + " bg-ink-50 text-ink-600"}>{s.personal.allTeachers}</div>
              </label>
              <div className="relative">
                <label className={labelCls}>{s.personal.searchClient}</label>
                <Search className="w-4 h-4 text-ink-400 absolute left-3.5 top-[calc(50%+6px)] -translate-y-1/2" />
                <input
                  disabled
              ...
              return (
                <div
                  ...
                  className={`bg-white rounded-xl shadow-xs overflow-hidden ${
                    isCurrentOrFuture ? "border-2 border-lavender-200" : "border border-ink-200"
                  }`}
                >
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full min-w-[960px] text-left">
                      <thead>
                        <tr className="border-b border-ink-100 text-[10px] uppercase tracking-wider text-ink-400 font-semibold">
                      ...
                      <tbody>
                        {dateLessons.map((lesson) => (
                          <tr key={lesson.id} className="border-b border-ink-50 text-xs text-ink-700">
                            ...
                            <td className="py-2.5 px-3 font-semibold text-ink-800">{lesson.clientDisplay}</td>
                            <td className="py-2.5 px-3 capitalize">{lesson.type}</td>
                            <td className="py-2.5 px-3">
                              <PaymentBadge lesson={lesson} locale={locale} />
                            </td>
                            <td className="py-2.5 px-3 font-semibold text-ink-800">{money(lesson.price)}</td>
                            <td className="py-2.5 px-3">
                              <AttendanceBadge lesson={lesson} locale={locale} />
                            </td>
                            <td className="py-2.5 px-3 text-ink-400">—</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
        ...
        <div
          className={`bg-white p-4 border border-ink-200 shadow-xs panel-card-stack panel-sell-under-tabs demo-field-disabled ${pageTabPanelCls("sell", "view")}`}
        >
          <div className="panel-form-header panel-form-header-wide-md mb-4">
            ...
            <div className="panel-form-header-text">
              <h2 className="text-base font-semibold tracking-tight text-ink-900">{ps.personalSellTitle}</h2>
              <p className="text-ink-400 text-[11px] leading-snug">{ps.personalSellSubtitle}</p>
            </div>
          </div>
          ...
          <button
            type="button"
            disabled
            className="mt-4 w-full max-w-md py-3 bg-gold-600/40 text-white text-xs font-semibold uppercase tracking-wider rounded-lg cursor-not-allowed"
          >
            {ps.saveLesson}
          </button>
          <p className="text-center text-[10px] text-ink-500 mt-2">{s.subs.demoHint}</p>
        </div>
      )}
    </div>
  );
