...
export function CrmCapabilities({ locale, t }: Props) {
  return (
    <section id="crm-sections" className="border-t border-ink-200 bg-white">
      <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-14">
        <h2 className="text-xl font-bold text-ink-900 sm:text-2xl">{t("crmCaps.title")}</h2>
        <p className="mt-2 text-sm text-ink-500">{t("crmCaps.subtitle")}</p>
        ...
        <dl className="mt-6 grid gap-x-8 gap-y-3 sm:grid-cols-2 lg:grid-cols-3">
          {capabilityItems.map(({ titleKey, descKey, icon: Icon }) => (
            <div key={titleKey} className="flex gap-2.5">
              <Icon className="mt-0.5 h-4 w-4 shrink-0 text-gold-500" strokeWidth={1.75} aria-hidden="true" />
              <div className="min-w-0">
                <dt className="text-sm font-medium text-ink-800">{t(titleKey)}</dt>
                <dd className="text-xs text-ink-500 leading-snug">{t(descKey)}</dd>
              </div>
            </div>
          ))}
        </dl>

        <p className="mt-6 flex gap-2.5 border-t border-ink-100 pt-5 text-sm text-ink-600">
          <Settings2 className="mt-0.5 h-4 w-4 shrink-0 text-gold-500" strokeWidth={1.75} aria-hidden="true" />
          <span>
            <span className="font-medium text-ink-800">{t("crmCaps.customizeTitle")}</span>
            {" — "}
