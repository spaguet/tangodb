      ...
      <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6 sm:py-20">
        ...
        <ul className="mt-8 grid gap-3 sm:grid-cols-2">
          {items.map(({ key, icon: Icon, iconBg, iconColor }) => (
            <li
              key={key}
              className="flex items-start gap-3 rounded-lg border border-ink-200 bg-white px-4 py-3.5"
            >
              <div
              ...
              <p className="pt-1.5 text-sm font-medium leading-snug text-ink-700">{t(key)}</p>
            </li>
          ))}
        </ul>
