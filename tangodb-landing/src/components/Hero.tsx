...
export function Hero({ locale, t }: Props) {
  return (
    <section className="relative overflow-hidden">
      <div className="absolute inset-0 -z-10 bg-gradient-to-br from-gold-50 via-white to-ink-50" />
      <div className="absolute top-0 right-0 -z-10 h-72 w-72 rounded-full bg-gold-100/40 blur-3xl" />
      <div className="absolute bottom-0 left-0 -z-10 h-64 w-64 rounded-full bg-gold-200/20 blur-3xl" />
      ...
      <div className="mx-auto max-w-6xl px-4 pt-8 pb-10 sm:px-6 sm:pt-12 sm:pb-12 lg:pt-14 lg:pb-14">
        <div className="grid items-center gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,28rem)] lg:gap-12 xl:grid-cols-[minmax(0,1fr)_32rem]">
          <div className="max-w-2xl animate-slide-up">
            <span className="inline-flex items-center gap-1.5 rounded-full border border-gold-200 bg-gold-50 px-3 py-1 text-xs font-semibold text-gold-700">
            ...
            <p className="mt-5 text-lg text-ink-600 leading-relaxed max-w-xl">{t("hero.subtitle")}</p>

            <CtaBlock locale={locale} t={t} className="mt-8" />

            <a
              href="#demo"
              className="mt-4 inline-block text-sm font-medium text-gold-700 underline-offset-4 transition-colors hover:text-gold-800 hover:underline"
